...
import { format, addMonths, subMonths, startOfMonth, endOfMonth, startOfWeek, endOfWeek, addDays, isSameMonth, isSameDay, isToday, addMinutes } from "date-fns";
import { es } from "date-fns/locale";
import { publicApi, EventTypeResolveResponse } from "@/lib/api/public";
...
import { Loader2, Calendar as CalendarIcon, Clock, ChevronLeft, ChevronRight, CheckCircle2, Globe } from "lucide-react";
...
export default function BookingPage({ params }: { params: Promise<{ tenant_slug: string, event_slug: string }> }) {
  ...
  useEffect(() => {
    publicApi.resolveEventType(tenant_slug, event_slug)
      .then(setData)
      .catch((err) => setError(err.message))
      .finally(() => setLoading(false));
  }, [tenant_slug, event_slug]);
  ...
  useEffect(() => {
      ...
      const fetchAvailability = async () => {
          ...
          try {
              const response = await publicApi.getEventTypeSlots(tenant_slug, event_slug, startStr, endStr);
  ...
  useEffect(() => {
    if (date && data) {
      ...
      publicApi.getEventTypeSlots(tenant_slug, event_slug, dateStr, dateStr)
        .then(setSlots)
      ...
      return (
          <div className="min-h-screen flex items-center justify-center bg-neutral-950 text-white p-4">
              <Card className="max-w-md w-full bg-neutral-900 border-neutral-800 text-neutral-100 p-8 text-center space-y-6">
                      ...
                      <div className="flex items-center gap-3">
                          ...
                          <div>
                              <p className="font-medium capitalize">
                                  {selectedSlot && format(new Date(selectedSlot), "EEEE d 'de' MMMM", { locale: es })}
                              </p>
                              <p className="text-sm text-neutral-400">
                                  {selectedSlot && format(new Date(selectedSlot), "HH:mm")} - {selectedSlot && format(new Date(new Date(selectedSlot).getTime() + data.event_type.duration * 60000), "HH:mm")}
                              </p>
                          </div>
                      </div>
                      <div className="flex items-center gap-3">
                  ...
                  {data.event_type.confirmation_button?.enabled && data.event_type.confirmation_button.url ? (
                      <Button 
           ...
           <div className="space-y-4">
              {data.tenant_avatar && (
                  <div className="relative w-16 h-16 rounded-full overflow-hidden border border-neutral-700">
                      <Image src={data.tenant_avatar} alt={data.tenant_name} fill className="object-cover" />
                  </div>
              )}
              <div>
                  <p className="text-neutral-400 font-medium text-sm uppercase tracking-wider">{data.tenant_name}</p>
                          ...
                          <div className="flex items-start gap-3 text-neutral-300">
                              ...
                              <div className="text-sm">
                                  <p className="font-medium capitalize text-neutral-200">
                                      {format(new Date(selectedSlot), "EEEE, d 'de' MMMM 'de' yyyy", { locale: es })}
                                  </p>
                                  <p className="text-neutral-400 mt-1">
                                      {format(new Date(selectedSlot), "HH:mm")} - {format(addMinutes(new Date(selectedSlot), data.event_type.duration), "HH:mm")}
                                  </p>
                              </div>
                          </div>
                ...
                <div className="flex-1 flex flex-col md:flex-row gap-8">
                   ...
                   <div className="flex-1">
                      ...
                      <Calendar
                        ...
                        startMonth={new Date()}
                        endMonth={addDays(new Date(), data.event_type.scheduling_limits.max_advance_days || 60)}
                        modifiers={{
                        ...
                        disabled={(date) => {
                            ...
                            const maxDate = addDays(today, data.event_type.scheduling_limits.max_advance_days || 60);
                   ...
                   {/* Slots Column */}
                   <div className="w-full md:w-48 space-y-4 border-l border-neutral-800 md:pl-8 min-h-[400px]">
                      <h3 className="text-md font-semibold mb-4 capitalize text-center">
                ...
                <div className="max-w-md mx-auto w-full animate-in fade-in slide-in-from-right-4 pt-4">
                   ...
                   <div className="space-y-6">
                      ...
                      <form onSubmit={handleBook} className="space-y-4">
                         <div className="space-y-2">
                           <Label htmlFor="name" className="text-neutral-300">Tu Nombre *</Label>
                           <Input 
                             id="name" 
                             required 
                             value={formData.name}
                             onChange={(e) => setFormData({...formData, name: e.target.value})}
                             className="h-10 bg-neutral-950 border-neutral-800 text-white placeholder:text-neutral-600 focus:border-neutral-700 focus:ring-1 focus:ring-neutral-700 rounded-md"
                           />
                         </div>
                         <div className="space-y-2">
                         ...
                         <div className="space-y-2">
                           <Label htmlFor="phone" className="text-neutral-300">Teléfono (Opcional)</Label>
                           <Input 
                             id="phone"
                             type="tel"
                         ...
                         <div className="space-y-2">
                           <Label htmlFor="notes" className="text-neutral-300">Notas Adicionales</Label>
                           <Textarea 
                             id="notes"
