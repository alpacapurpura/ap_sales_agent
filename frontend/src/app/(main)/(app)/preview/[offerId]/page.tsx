...
async function getLandingPageConfig(offerId: string): Promise<LandingPageConfig | null> {
  try {
...
export default async function LandingPreviewPage({ params }: { params: Promise<{ offerId: string }> }) {
  const { offerId } = await params;
  const landing = await getLandingPageConfig(offerId);
    ...
    return (
        <div className="flex items-center justify-center min-h-screen bg-slate-50">
            <div className="text-center p-8">
                ...
                <p className="text-slate-600">No se pudo cargar la configuración de la landing. Asegúrate de haber guardado los cambios.</p>
            </div>
        </div>
    );
  }
  ...
  return (
    <div className="pt-8"> {/* Padding for banner */}
        <PreviewBanner />
        {template}
    </div>
  );
