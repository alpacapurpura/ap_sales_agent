...
import { format, startOfWeek, endOfWeek, addWeeks, subWeeks, addDays, isSameDay, differenceInMinutes } from "date-fns";
...
function AppointmentsView() {
  ...
  return (
    <div className="space-y-4">
        <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-4">
                <div className="flex items-center space-x-2">
                    <Switch id="weekends" checked={showWeekends} onCheckedChange={setShowWeekends} />
                    <Label htmlFor="weekends" className="text-sm font-medium">Fines de semana</Label>
                </div>
        ...
        <div className={`grid grid-cols-1 gap-4 ${showWeekends ? 'md:grid-cols-7' : 'md:grid-cols-5'}`}>
            {days.map((day) => (
                    ...
                    <CardContent className="p-2 flex-1 space-y-2">
                        {appointments
                            ...
                            .map(appt => {
                                ...
                                // Try to extract lead name from "Title - Name" or "Reunión con Name"
                                let leadName = appt.summary;
                                if (appt.summary.includes(" - ")) {
                                    leadName = appt.summary.split(" - ").pop();
                                } else if (appt.summary.includes(" con ")) {
                                    leadName = appt.summary.split(" con ").pop();
                                }
                                ...
                                return (
                                    <div 
                                        ...
                                        className="group p-2 text-xs bg-background border border-border rounded-md cursor-pointer hover:border-primary/50 hover:shadow-sm transition-all"
                                    >
                                        <div className="font-medium truncate text-foreground mb-1.5" title={appt.summary}>
                                            {leadName}
                                        </div>
                                        <div className="flex items-center justify-between gap-2">
                                            <div className="px-1.5 py-0.5 rounded-[4px] bg-muted group-hover:bg-muted/80 text-[10px] font-medium text-muted-foreground whitespace-nowrap">
                                                {duration} min
                                            </div>
