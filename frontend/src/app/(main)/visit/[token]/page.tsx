...
import { Loader2, Calendar as CalendarIcon, Clock, CheckCircle2, Video, Globe, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function PublicVisitPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  useEffect(() => {
    if (date && data) {
      ...
      publicApi.getSlots(token, dateStr, dateStr)
        .then(setSlots)
          ...
          <CardContent>
             <div className="bg-black/40 p-4 rounded-lg mb-6 border border-green-900/30">
               ...
               <p className="text-green-500/80">
                 {format(new Date(selectedSlot!), "h:mm a", { locale: es })} - {format(new Date(new Date(selectedSlot!).getTime() + 30*60000), "h:mm a")}
               </p>
             </div>
             <Button onClick={() => window.location.reload()} variant="outline" className="w-full border-white/10 hover:bg-white/5 text-white hover:text-white">
      ...
      <Card className="w-full max-w-5xl shadow-2xl overflow-hidden border border-white/10 bg-black/40 backdrop-blur-xl">
        <div className="flex flex-col md:flex-row min-h-[600px]">
          ...
          <div className="w-full md:w-[300px] p-6 border-b md:border-b-0 md:border-r border-white/10 bg-black/20 flex flex-col gap-6">
             {step === "form" && (
                <Button variant="ghost" size="icon" className="md:hidden self-start -ml-2 mb-2 text-white hover:bg-white/10" onClick={() => setStep("date")}>
             ...
             <div className="space-y-4">
                ...
                <div>
                   ...
                   <h1 className="text-2xl font-bold text-white leading-tight">Sesión de Estrategia</h1>
                </div>
                   ...
                   <div className={cn(
                      ...
                      <Calendar
                        ...
                        classNames={{
                           ...
                           head_row: "flex",
                           head_cell: "text-neutral-500 rounded-md w-12 font-normal text-[0.8rem] h-10 uppercase",
                           ...
                           day_selected: "bg-white !text-black hover:bg-white/90 focus:bg-white/90 font-bold",
                ...
                <div className="flex-1 p-6 md:p-12 animate-in fade-in slide-in-from-right-4">
                   ...
                   <div className="max-w-md">
                      ...
                      <p className="text-sm text-neutral-400 mb-8">
                         Para agendar el {format(new Date(selectedSlot!), "EEEE d 'de' MMMM 'a las' h:mm a", { locale: es })}
                      </p>
                      ...
                      <form onSubmit={handleBook} className="space-y-5">
                         ...
                         <div className="pt-4">
                           <Button type="submit" disabled={bookingLoading} className="w-full h-11 text-base bg-white text-black hover:bg-white/90">
