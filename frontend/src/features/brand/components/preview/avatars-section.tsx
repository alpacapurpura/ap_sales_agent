                ...
                {safeAvatars.map((avatar: any) => {
                    ...
                    <div key={avatar.id} className="flex flex-col items-center text-center group/avatar cursor-pointer" onClick={() => onEdit(avatar)}>
                        <div className="relative mb-3">
                            ...
                            {/* Status Indicator */}
                            <div className={`absolute bottom-0 right-0 h-4 w-4 rounded-full border-2 border-background ${isComplete ? 'bg-green-500' : 'bg-amber-500'}`} title={isComplete ? "Completo" : "Faltan datos"} />
                        </div>
