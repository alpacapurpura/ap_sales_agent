      ...
      {!hasContent ? (
        ...
        <div className="pl-0 md:pl-14">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-8">
                {team.map((member) => (
                    <div key={member.id} className="flex flex-col items-center text-center group/member cursor-pointer" onClick={() => onEdit(member)}>
                        <div className="relative mb-3">
                            <Avatar className="h-20 w-20 border-2 border-transparent group-hover/member:border-primary transition-colors">
                                <AvatarImage 
                                    src={member.headshot_url ? (member.headshot_url.startsWith("http") ? member.headshot_url : `${apiUrl}${member.headshot_url}`) : undefined} 
                            ...
                            {member.is_primary_voice && (
                                <div className="absolute -top-1 -right-1 bg-background rounded-full p-1 shadow-sm border border-border" title="Voz Principal">
                                    <Star className="w-3 h-3 fill-yellow-400 text-yellow-400" />
                                </div>
                            )}
                        </div>
                        <h4 className="font-medium text-foreground text-sm truncate w-full px-2">{member.name}</h4>
