...
describe('Brand Section Config', () => {
  ...
  describe('BRAND_SECTION_ORDER', () => {
    it('should list all sections in order', () => {
      expect(BRAND_SECTION_ORDER).toEqual([
        'esencia', 'estrategia', 'publico', 'identidad-creativa',
      ]);
    });
  });
  ...
  describe('EDIT_MODE_META', () => {
    it('should have metadata for all edit modes', () => {
      const modes = [
        'none', 'identity', 'voice', 'legal', 'authority', 'team',
        'testimonials', 'avatars', 'contact', 'visuals', 'visuals-wizard',
        'logos', 'story', 'methodology', 'positioning', 'values-essence',
        'storybrand', 'communication-assets',
      ];
