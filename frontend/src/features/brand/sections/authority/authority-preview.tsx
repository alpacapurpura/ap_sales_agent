...
interface TrustSectionProps {
  ...
  onEditIdentity: () => void;
  onEditAuthority: (item?: AuthorityItem) => void;
}
...
const AuthorityBadge = ({
  ...
  >
    ...
    <div className="flex-1 min-w-0">
      <h5 className="text-sm font-medium text-foreground truncate group-hover/badge:text-primary transition-colors">
        {item.entity_name}
      </h5>
      <p className="text-xs text-muted-foreground truncate">{item.context || item.type}</p>
    </div>
    {item.proof_url && (
      <ExternalLink className="w-3 h-3 text-muted-foreground ml-2 opacity-0 group-hover/badge:opacity-50 transition-opacity" />
    )}
  </div>
);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Premios, certificaciones y menciones en medios.
              </p>
            </div>
          </div>
