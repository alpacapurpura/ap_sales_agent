            ...
            {!hasContent ? (
                <div className="pl-0 md:pl-14 cursor-pointer" onClick={onEdit}>
                    <p className="text-lg text-muted-foreground italic mb-2">
                        &quot;Define los conceptos creativos y activos que dan vida a tu marca en cada etapa del funnel.&quot;
                    </p>
                            ...
                            <div className="flex gap-3 overflow-x-auto pb-2">
                                {creative_concepts.map((concept) => (
                                    ...
                                    >
                                        ...
                                        {concept.description && (
                                            <p className="text-xs text-muted-foreground mt-2 line-clamp-2">{concept.description}</p>
                                        )}
                                    </div>
                                ))}
                            </div>
                            ...
                            {(["TOFU", "MOFU", "BOFU", "retention"] as const).map((stage) => {
                                ...
                                return (
                                    <div key={stage} className="space-y-2">
                                        <div className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold ${STAGE_COLORS[stage] || "bg-muted text-muted-foreground"}`}>
                                                ...
                                                {stageAssets.map((asset) => (
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-1.5 mb-1.5">
                                                            ...
                                                            {asset.status && (
                                                                <Badge variant={STATUS_VARIANTS[asset.status] || "outline"} className="text-[10px] h-5 px-1.5">
                                                                    {STATUS_LABELS[asset.status] || asset.status}
                                                                </Badge>
                                                            )}
                                                        </div>
                                                        <p className="font-medium text-sm truncate">{asset.title || "Sin titulo"}</p>
                                                        {asset.objective && (
                                                            <p className="text-xs text-muted-foreground mt-1">{asset.objective}</p>
                                                        )}
                                                    </div>
                                                ))}
                                            </div>
