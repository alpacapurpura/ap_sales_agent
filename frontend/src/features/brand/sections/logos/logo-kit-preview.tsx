...
export function LogoKitPreview({ visuals, onEdit }: LogoKitPreviewProps) {
    ...
    const logoSlots: { key: keyof BrandLogos; label: string; bg: string; minW: string; aspect?: string }[] = [
        { key: "primary", label: "Principal", bg: "bg-white", minW: "min-w-[140px]", aspect: "aspect-video" },
        { key: "secondary", label: "Icono", bg: "bg-muted/20", minW: "w-20", aspect: "aspect-square" },
        { key: "dark_mode", label: "Fondo Oscuro", bg: "bg-slate-950", minW: "min-w-[140px]", aspect: "aspect-video" },
        { key: "light_mode", label: "Fondo Claro", bg: "bg-white", minW: "min-w-[140px]", aspect: "aspect-video" },
    ];
                        ...
                        {logoSlots.map((slot) => {
                            ...
                            return (
                                <div key={slot.key} className="space-y-2">
                                    <div className={`relative h-20 px-4 py-2 border rounded-lg ${slot.bg} flex items-center justify-center ${slot.minW}`}>
                                    ...
                                    <p className="text-[10px] text-muted-foreground font-mono uppercase">{slot.label}</p>
                                </div>
                            );
                        })}
                    </div>
