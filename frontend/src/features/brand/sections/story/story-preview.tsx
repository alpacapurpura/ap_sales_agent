...
import type { BrandStory, BrandVisuals } from "@/features/brand/types";
...
export function StorySection({ story, visuals, onEdit }: StorySectionProps) {
  ...
  const hasContent =
    story?.origin_story || story?.mission || story?.vision || milestones.length > 0;
            ...
            <div className="border-l-2 border-border pl-6 space-y-8 relative py-2">
              {milestones.map((milestone) => (
                <div key={milestone.id} className="relative">
                  ...
                  <span className="text-xs font-mono text-muted-foreground block mb-1 font-bold">
                    {milestone.year}
                  </span>
