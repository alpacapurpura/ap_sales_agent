...
export function TraceInspector({ event, onClose }: TraceInspectorProps) {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent
        className="w-[800px] sm:w-[600px] overflow-hidden flex flex-col p-0 gap-0"
        side="right"
      >
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-1">
        ...
        <div className="p-3 bg-muted rounded-md text-sm whitespace-pre-wrap">{event.content}</div>
      </div>
      <div className="grid gap-1">
    ...
    <Tabs defaultValue="state" className="w-full">
      <TabsList className="w-full">
        ...
        <TabsTrigger value="llm" className="flex-1">
          LLM Logs ({detail.llm_logs?.length || 0})
        </TabsTrigger>
      </TabsList>
            ...
            {detail.llm_logs.map((log) => (
              <AccordionItem key={log.id} value={log.id}>
                <AccordionTrigger className="text-sm">
                  {log.model} - {log.prompt_template || "Raw Prompt"} (
