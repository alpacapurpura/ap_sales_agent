...
export function getNodeIcon(nodeName: string | undefined, hasLLM: boolean = false) {
  ...
  if (name.includes("retrieve") || name.includes("search") || name.includes("knowledge")) return Database;
  if (name.includes("generate") || name.includes("llm") || name.includes("answer")) return Sparkles;
  if (name.includes("check") || name.includes("grade") || name.includes("validate")) return ShieldCheck;
