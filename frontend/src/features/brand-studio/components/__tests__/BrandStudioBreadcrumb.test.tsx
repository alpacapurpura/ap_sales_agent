...
// Fase 03 · Block D — the resolver is now injected (comes from
// ``useBrandSectionLabelResolver`` which reads the BE catalog). Tests
// stub it with the subset of labels we assert on.
const STUB_LABELS: Record<string, string> = {
  positioning: "Posicionamiento",
  publico: "Buyer personas",
};
...
describe("BrandStudioBreadcrumb · buildCrumbs", () => {
  it("renders the root when path ends at /brand-studio", () => {
    const crumbs = buildCrumbs("/t-demo/brand-studio", "t-demo", resolveLabel);
    expect(crumbs).toHaveLength(1);
    expect(crumbs[0]?.label).toBe("Brand Studio");
  });

  it("appends the section when present", () => {
    const crumbs = buildCrumbs("/t/brand-studio/positioning", "t", resolveLabel);
    expect(crumbs.map((c) => c.label)).toEqual(["Brand Studio", "Posicionamiento"]);
  });

  it("appends the field id on singleton routes", () => {
    const crumbs = buildCrumbs("/t/brand-studio/positioning/uvp", "t", resolveLabel);
  ...
  it("includes instance and field for uniform collection route", () => {
    const crumbs = buildCrumbs(
      "/t/brand-studio/publico/instance/alicia-1234-abcd/dolor",
      "t",
      resolveLabel,
    );
  ...
  it("supports legacy publico/persona alias", () => {
    const crumbs = buildCrumbs(
      "/t/brand-studio/publico/persona/p-1234-abcd/pain",
      "t",
      resolveLabel,
    );
    expect(crumbs).toHaveLength(4);
