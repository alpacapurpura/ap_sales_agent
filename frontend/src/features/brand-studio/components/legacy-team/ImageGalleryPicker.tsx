...
/**
 * LEGACY picker used only by the ``LegacyTeamManager`` adapter (see index).
 * The canonical form-runtime action lives at
 * ``brand-studio/actions/ImageGalleryPickerAction.tsx``. This file accepts a
 * separate ``primaryImage`` / ``onPrimaryChange`` pair because the legacy
...
export function ImageGalleryPicker({
  ...
  const handleConfirmSelection = () => {
    ...
    if (primaryImage && !tempSelected.includes(primaryImage)) {
      onPrimaryChange(tempSelected.length > 0 ? tempSelected[0] : "");
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <span className="text-sm font-medium">
          Galería Personal ({images.length}/{maxImages})
        </span>
        <Button
          type="button"
          variant="outline"
                ...
                {uploadFile && (
                  <div className="space-y-4 bg-muted/30 p-4 rounded-lg border">
                    ...
                    <div className="space-y-2">
                      <label htmlFor="legacy-team-upload-desc" className="text-xs font-medium">
                        Descripción (Opcional)
                      </label>
                      <Textarea
                        id="legacy-team-upload-desc"
