...
import {
  ...
  getExpandedMetrics,
} from "../lib/channel-display-registry";
...
describe("getSummaryMetrics", () => {
  ...
  it("preserves registry-defined order, not backend order", () => {
    ...
    // Registry order: ig_views, reach, total_interactions, ig_followers_count
    expect(result[0].name).toBe("ig_views");
  ...
  it("returns empty array when no data matches config", () => {
    const noMatchMetrics: MetricValue[] = [{ name: "unrelated", value: 42 }];
...
describe("getPrimaryMetricSpec", () => {
  it("returns registry primary for known channels", () => {
    expect(getPrimaryMetricSpec("ig-organic", "organic_social")).toEqual({
      name: "reach",
      label: "alcance",
    });

    expect(getPrimaryMetricSpec("meta-ads", "paid")).toEqual({
      name: "impressions",
      label: "impresiones",
    });

    expect(getPrimaryMetricSpec("yt-organic", "organic_social")).toEqual({
      name: "views",
      label: "vistas",
    });

    expect(getPrimaryMetricSpec("website-total", "website")).toEqual({
      name: "sessions",
      label: "sesiones",
    });
  });
  ...
  it("falls back to channelType for unknown channels", () => {
    expect(getPrimaryMetricSpec("unknown-paid", "paid")).toEqual({
      name: "impressions",
      label: "impresiones",
    });

    expect(getPrimaryMetricSpec("unknown-organic", "organic_social")).toEqual({
      name: "reach",
      label: "alcance",
    });

    expect(getPrimaryMetricSpec("unknown-outbound", "outbound")).toEqual({
      name: "contacts",
      label: "contactos",
    });
  });
  ...
  it("defaults to leads for unknown channel types", () => {
    expect(getPrimaryMetricSpec("unknown", "something_new")).toEqual({
      name: "leads",
      label: "leads",
    });
  });
    ...
    for (const [slug, config] of Object.entries(CHANNEL_DISPLAY_REGISTRY)) {
      if (config.layout === "expanded") {
        expect(
          ...
          `${slug} with layout=expanded must have expandedMetrics`,
        ).toBeDefined();
