              ...
              <div>
                ...
                <div className="rounded-lg border bg-card p-3 space-y-1">
                  {[...campaignData.campaigns]
                    ...
                    .slice(0, 3)
                    .map(camp => (
                      <div key={camp.externalId} className="flex items-center justify-between py-1.5 border-b border-border/50 last:border-0">
                        <div className="flex items-center gap-2">
                          <span className={cn(
                            'w-1.5 h-1.5 rounded-full',
                            camp.effectiveStatus === 'ACTIVE' ? 'bg-emerald-500' :
                            camp.health === 'critical' ? 'bg-red-500' :
                            camp.effectiveStatus === 'PAUSED' ? 'bg-zinc-500' : 'bg-amber-500'
                          )} />
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <span className={cn(
                            ...
                            (camp.metrics.roas ?? 0) >= 2 ? 'text-emerald-500' :
                            (camp.metrics.roas ?? 0) >= 1 ? 'text-amber-500' : 'text-red-500'
                          )}>
