...
function makeCampaign(
  overrides: Partial<CampaignWithMetrics> = {},
): CampaignWithMetrics {
  return {
    externalId: 'c1',
...
function makeRec(
  overrides: Partial<CampaignRecommendation> = {},
): CampaignRecommendation {
  return {
...
describe('useMetaAdsNotices', () => {
  ...
  it('maps a Meta campaign recommendation to the "campanas" tab', () => {
    ...
    expect(result.current.byTab.campanas[0].contextLabel).toContain(
      'Campaña Activa 1',
    );
  });
  ...
  it('categorizes audience recommendations to "audiencia"', () => {
    const campaignData = makeCampaignData({
      recommendations: [
        makeRec({ recommendationType: 'AUDIENCE_OVERLAP', objectIds: ['c1'] }),
      ],
    });
  ...
  it('categorizes bid/cost recommendations to "costos"', () => {
    const campaignData = makeCampaignData({
      recommendations: [
        makeRec({ recommendationType: 'BID_STRATEGY', objectIds: ['c1'] }),
      ],
    });
  ...
  it('merges structural recommendations related to active campaigns', () => {
    ...
    const healthCheck = makeHealthCheck({
      recommendations: [
        makeStructuralRec({ severity: 'critical', relatedTargetId: 'c1' }),
      ],
    });

    const { result } = renderHook(() =>
      useMetaAdsNotices({ campaignData, healthCheck }),
    );
  ...
  it('sorts notices by severity within each tab (critical first)', () => {
    ...
    expect(result.current.byTab.campanas.map(n => n.title)).toEqual([
      'rejected',
      'low-delivery',
    ]);
  });
  ...
  it('exposes a stable dedupe id that survives re-fetches', () => {
    ...
    expect(r1.current.byTab.creativos[0].id).toBe(
      r2.current.byTab.creativos[0].id,
    );
  });
});
