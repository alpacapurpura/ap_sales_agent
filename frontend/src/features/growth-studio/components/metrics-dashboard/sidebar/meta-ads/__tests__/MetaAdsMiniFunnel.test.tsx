...
const STEPS: FunnelStep[] = [
  ...
  { label: "Clics", metricName: "clicks", value: 518, conversionRate: 4.2 },
  {
    ...
    metricName: "meta_landing_page_views",
    value: 352,
    conversionRate: 68.0,
  },
...
describe("MetaAdsMiniFunnel — empty state", () => {
  ...
  it("renders the all-filter empty state by default", () => {
    ...
    expect(screen.getByText(/sin actividad medible en este período/i)).toBeInTheDocument();
  });

  it('renders the offer-specific empty state when filter="offer"', () => {
    renderFunnel(<MetaAdsMiniFunnel steps={ZERO_STEPS} filter="offer" />);
    expect(screen.getByText(/esta offer aún no tiene campañas asignadas/i)).toBeInTheDocument();
  });
  ...
  it('renders the branding-specific empty state when filter="branding"', () => {
    ...
    expect(screen.getByText(/sin impresiones de branding en este período/i)).toBeInTheDocument();
  });
  ...
  it('renders the unassigned-specific empty state when filter="unassigned"', () => {
    ...
    expect(screen.getByText(/las campañas sin asignar no reportaron eventos/i)).toBeInTheDocument();
  });
  ...
  it("does NOT render empty state if any step has data", () => {
    ...
    renderFunnel(<MetaAdsMiniFunnel steps={steps} />);
    expect(screen.queryByText(/sin actividad medible en este período/i)).not.toBeInTheDocument();
