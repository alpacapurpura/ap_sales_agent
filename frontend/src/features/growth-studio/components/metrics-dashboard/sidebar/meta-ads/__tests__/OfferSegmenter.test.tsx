...
describe("OfferSegmenter", () => {
  ...
  describe("chip kind markers", () => {
    ...
    it("tags each chip with a data-chip-kind attribute", () => {
      ...
      expect(screen.getByRole("button", { name: /MasterClass/i })).toHaveAttribute(
        "data-chip-kind",
        "offer",
      );
      expect(
  ...
  describe("keyboard navigation (roving tabindex)", () => {
    ...
    it("uses roving tabindex: only the selected chip is tabbable", () => {
      renderNav("off-1");
      expect(screen.getByRole("button", { name: /Todas/i })).toHaveAttribute("tabindex", "-1");
      expect(screen.getByRole("button", { name: /MasterClass/i })).toHaveAttribute("tabindex", "0");
      expect(screen.getByRole("button", { name: /Servicio/i })).toHaveAttribute("tabindex", "-1");
    });
    ...
    it("defaults tabindex=0 to the first chip when selection is not in the list", () => {
      ...
      expect(screen.getByRole("button", { name: /Todas/i })).toHaveAttribute("tabindex", "0");
    });
    ...
    it("ArrowDown also moves focus to the next chip", () => {
      ...
      expect(screen.getByRole("button", { name: /MasterClass/i })).toHaveFocus();
    });
