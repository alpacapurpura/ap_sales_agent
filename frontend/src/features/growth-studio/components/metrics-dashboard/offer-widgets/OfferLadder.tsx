...
function OfferCard({
  ...
  const themeClasses = {
    blue: {
      wrapper:
        "hover:bg-blue-50/50 hover:border-blue-300 dark:hover:bg-blue-900/10 dark:hover:border-blue-700",
      bar: "bg-slate-200 group-hover:bg-blue-400 dark:bg-slate-700",
    },
    indigo: {
      wrapper:
        "hover:bg-indigo-50/50 hover:border-indigo-300 dark:hover:bg-indigo-900/10 dark:hover:border-indigo-700",
      bar: "bg-slate-200 group-hover:bg-indigo-400 dark:bg-slate-700",
    },
    amber: {
      wrapper:
        "hover:bg-amber-50/50 hover:border-amber-300 dark:hover:bg-amber-900/10 dark:hover:border-amber-700",
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start">
        <div className="pl-2">
          <h5
            className="font-semibold text-sm text-slate-800 dark:text-slate-200 leading-tight truncate max-w-[140px] sm:max-w-[160px]"
            title={offer.publicName}
          >
            {offer.publicName}
          </h5>
      ...
      <div className="flex items-end justify-between border-b border-border pb-2">
        <div>
          <h3 className="font-semibold text-xl flex items-center text-foreground">
            <Layers className="w-5 h-5 mr-2 text-emerald-600 dark:text-emerald-500" /> Desglose por
            Offer Ladder
          </h3>
      ...
      <div className="mt-6 bg-slate-800 dark:bg-slate-900 rounded-xl p-4 flex flex-col md:flex-row gap-6 items-center justify-between text-white shadow-md border border-slate-700">
              ...
              >
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 mt-1">
                    {Object.entries(offer.sourceBreakdown || {})
                      .map(([src, count]) => `${src}: ${count}`)
                      .join(", ") || "Sin datos"}
                  </p>
                </div>
                <span className="font-bold text-emerald-400">
                  {formatMoney(offer.totalRevenue, currency, { fractionDigits: 0 })}
                </span>
              </div>
            ))
          ) : (
        ...
        <div className="text-right md:w-1/4">
          <div className="text-2xl font-black text-emerald-400">
            {formatCompactMoney(
              corporativoOffers.reduce((s, o) => s + o.totalRevenue, 0),
              currency,
            )}
          </div>
          <p className="text-xs text-slate-400 mt-1">
            {corporativoOffers.reduce((s, o) => s + o.salesCount, 0)} ventas
          </p>
        </div>
      </div>
    </div>
