...
export function OfferHealthCard({ offer }: OfferHealthCardProps) {
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-4 shadow-sm hover:border-emerald-300 dark:hover:border-emerald-700 hover:shadow-md transition-all group relative overflow-hidden h-full flex flex-col">
      <div
        className={`absolute top-0 left-0 w-1 h-full transition-colors bg-slate-200 dark:bg-slate-700 ${isHealthy ? "group-hover:bg-emerald-400" : "group-hover:bg-amber-400"}`}
      ></div>
      ...
      <div className="flex justify-between items-start mb-3">
        <div className="pl-2 flex-1">
          ...
          <p className="text-[10px] text-muted-foreground uppercase tracking-wider mt-1">
            {offer.totalCustomers} Clientes Totales
          </p>
        </div>
        <span
      ...
      <div className="pl-2 space-y-3 flex-1 flex flex-col justify-end">
        ...
        <div className="w-full h-1.5 rounded-full bg-muted overflow-hidden flex">
          <div
            className={`h-full ${isHealthy ? "bg-emerald-500" : "bg-amber-500"}`}
            style={{ width: `${offer.healthPct}%` }}
          ></div>
        </div>
        ...
        {offer.ttvDays != null && (
          <div className="pt-2 border-t border-border mt-auto">
            <p className="text-[10px] text-muted-foreground flex items-center">
              ...
              Tiempo de activación: {Math.round(offer.ttvDays)}{" "}
              {Math.round(offer.ttvDays) === 1 ? "día" : "días"}
            </p>
          </div>
        )}
      </div>
