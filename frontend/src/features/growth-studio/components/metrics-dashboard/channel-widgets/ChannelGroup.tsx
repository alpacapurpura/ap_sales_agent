...
export function buildSummary(groupType: GroupType, totals: Record<string, number>, channelCount: number, currency = 'USD'): string {
  switch (groupType) {
    case 'organic_social':
    ...
    case 'retargeting': {
      ...
      const costStr = costPerMql != null ? ` | Costo/MQL: ${formatMoney(costPerMql, currency, { fractionDigits: 2 })}` : '';
...
export function ChannelGroup({ title, totals, channels, groupType, defaultOpen = true, stageId, onMetricClick, onChannelClick, onConfigure }: ChannelGroupProps) {
