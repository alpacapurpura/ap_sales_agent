...
export const SalesDetail = React.memo(function SalesDetail({ onMetricClick }: SalesDetailProps) {
  ...
  const oppToSalesConvRate = useMemo(() => {
    ...
    return (
      ...
      (data.headerKpis.newCustomers > 0 && data.miniFunnel.sourceValue > 0
        ? (data.headerKpis.newCustomers / data.miniFunnel.sourceValue) * 100
        : 0)
    );
  }, [data]);
  ...
  const handleKpiClick = useCallback(
    (metricName: string, currentValue: number) => {
      if (!data) return;
      onMetricClick?.({
        stageId: "VENTAS",
        channelSlug: "shopify",
    ...
    <div className="space-y-12 animate-fade-in bg-background p-6 rounded-2xl text-foreground border border-border">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Ingresos generados (Revenue), clientes nuevos y costo de adquisición por oferta.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button
            variant={shopifyError ? "destructive" : "outline"}
      ...
      <div className="bg-card rounded-xl p-6 shadow-sm border border-border relative overflow-hidden">
        <h2 className="text-lg font-semibold mb-6 flex items-center text-foreground/90">
          <Banknote className="w-5 h-5 mr-2 text-emerald-600 dark:text-emerald-500" /> Rendimiento
          de Ventas
        </h2>
        ...
        <div className="relative mb-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 relative z-10 w-full">
            ...
            >
              <span className="text-[10px] text-emerald-300 font-bold uppercase mb-1">
                Revenue Total
              </span>
              <span className="text-4xl font-black text-white">
                {formatDualCurrency(
                  headerKpis.totalRevenue,
                  headerKpis.currency,
                  headerKpis.totalRevenueUsd,
                ).replace(/ \(.+\)/, "")}
              </span>
              ...
              {headerKpis.totalRevenueUsd && headerKpis.currency !== "USD" && (
                <div className="mt-3 text-xs font-medium text-emerald-300">
                  ...
                  {new Intl.NumberFormat("en-US", {
                    ...
                    minimumFractionDigits: 0,
                    maximumFractionDigits: 0,
                  }).format(headerKpis.totalRevenueUsd)}{" "}
                  USD)
                </div>
              )}
            </button>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            <div className="bg-background p-4 rounded-lg border border-border text-center">
              ...
              <p className="text-2xl font-bold text-foreground mt-1">
                {new Intl.NumberFormat("es-MX", {
                  style: "currency",
                  currency: headerKpis.shopifyCurrency,
                  minimumFractionDigits: 0,
                  maximumFractionDigits: 0,
                }).format(headerKpis.shopifyRevenue)}
              </p>
            </div>
            <div className="bg-background p-4 rounded-lg border border-border text-center">
              <span className="text-[10px] text-muted-foreground font-bold uppercase">Pedidos</span>
              <p className="text-2xl font-bold text-foreground mt-1">
                {headerKpis.shopifyOrderCount.toLocaleString("es-ES")}
              </p>
            </div>
            <div className="bg-background p-4 rounded-lg border border-border text-center">
              ...
              <p className="text-2xl font-bold text-foreground mt-1">
                {new Intl.NumberFormat("es-MX", {
                  ...
                  minimumFractionDigits: 0,
                  maximumFractionDigits: 0,
                }).format(headerKpis.shopifyAvgOrderValue)}
              </p>
            </div>
            <div className="bg-background p-4 rounded-lg border border-border text-center">
              <span className="text-[10px] text-muted-foreground font-bold uppercase">
            ...
            <div className="flex flex-col items-center p-2 rounded-md bg-muted/50">
              ...
              <span className="font-semibold text-foreground">
                {new Intl.NumberFormat("es-MX", {
                  ...
                  minimumFractionDigits: 0,
                  maximumFractionDigits: 0,
                }).format(headerKpis.totalDiscounts)}
              </span>
            </div>
            <div className="flex flex-col items-center p-2 rounded-md bg-muted/50">
              <span className="text-[10px] text-muted-foreground font-medium uppercase">
            ...
            <div className="flex flex-col items-center p-2 rounded-md bg-muted/50">
              ...
              <span className="font-semibold text-foreground">
                {new Intl.NumberFormat("es-MX", {
                  ...
                  minimumFractionDigits: 0,
                  maximumFractionDigits: 0,
                }).format(headerKpis.shippingRevenue)}
              </span>
            </div>
            <div className="flex flex-col items-center p-2 rounded-md bg-muted/50">
              <span className="text-[10px] text-muted-foreground font-medium uppercase">
              ...
              <span className="font-semibold text-foreground">
                {headerKpis.refundCount} (
                {new Intl.NumberFormat("es-MX", {
                  style: "currency",
                  currency: headerKpis.shopifyCurrency,
                  minimumFractionDigits: 0,
                  maximumFractionDigits: 0,
                }).format(headerKpis.refundAmount)}
                )
              </span>
            </div>
            <div className="flex flex-col items-center p-2 rounded-md bg-muted/50">
      ...
      <OfferLadder adquisicion={adquisicion} expansion={expansion} onMetricClick={onMetricClick} />
    </div>
  );
});
