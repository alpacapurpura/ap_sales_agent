...
export const NurtureOpportunityDetail = React.memo(function NurtureOpportunityDetail({
  onMetricClick,
  onChannelClick,
  onConfigure,
  onDisconnectedClick,
  onNoDataClick,
}: NurtureOpportunityDetailProps) {
  const { timezone } = useTenantLocale();
  const {
    data: nurtureData,
    isLoading: nurtureLoading,
    error: nurtureError,
    refetch: refetchNurture,
  } = useNurtureDetail();
  const {
    data: oppData,
  ...
  const { totalNurtureLeads, totalMqls, totalNurtureSpend, totalSqls, oppConvRate, costPerSql } =
    useMemo(() => {
      if (!nurtureData || !oppData)
        return {
          totalNurtureLeads: 0,
          totalMqls: 0,
          totalNurtureSpend: 0,
  ...
  const activeBottlenecks = useMemo(
    () => oppData?.bottlenecks.filter((b) => b.severity !== "normal") ?? [],
    [oppData],
  );
  ...
  if (nurtureError || oppError) {
    return (
      <DetailError
        error={
          nurtureError instanceof Error
            ? nurtureError
            : oppError instanceof Error
              ? oppError
  ...
  const NurtureChannelRow = ({
    ...
    if (
      channel.slug.includes("meta") ||
      channel.slug.includes("fb") ||
      channel.slug.includes("ig") ||
      channel.slug.includes("instagram")
    ) {
      Icon = <BrandIcon name={channel.slug} className="w-5 h-5" />;
    } else if (
      channel.slug.includes("whatsapp") ||
      channel.slug.includes("wa") ||
      channel.slug.includes("nico-agent")
    ) {
      Icon = (
        <BrandIcon
          name={channel.slug.includes("nico-agent") ? "nicolify" : channel.slug}
          className="w-5 h-5"
        />
      );
    ...
    const mainVal =
      getMetric(channel.metrics, defaultMetricName) ||
      getMetric(channel.metrics, "total_mqls") ||
      getMetric(channel.metrics, "total_sqls") ||
      channel.value ||
      0;
    ...
    return (
      ...
      >
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 rounded flex items-center justify-center font-bold ${bgColor}`}>
            {Icon}
          </div>
          <div>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground flex items-center gap-2">
            ...
            <span className="px-2 py-0.5 mt-1 rounded-full bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400 text-xs font-medium tracking-wide">
              ETAPA 3 y 4
            </span>
          </h1>
          <p className="text-muted-foreground text-sm mt-1">
            Calentamiento de leads (MQLs) y generación de intenciones de compra (SQLs).
          </p>
        </div>
        <div className="flex items-center gap-3">
          <p className="text-xs text-muted-foreground italic mr-2">
            Actualizado:{" "}
            {nurtureData.lastUpdated ? formatTenantTime(nurtureData.lastUpdated, timezone) : "Hoy"}
          </p>
        ...
        {totalNurtureLeads === 0 && totalMqls === 0 && totalSqls === 0 ? (
          <div className="text-center py-12 px-4 rounded-xl border-2 border-dashed border-border bg-muted/50">
            ...
            <p className="text-muted-foreground max-w-sm mx-auto mb-4">
              No hay datos suficientes para mostrar el termómetro de ventas.
            </p>
          </div>
        ) : (
          <div className="relative mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 relative z-10 w-full mb-6">
              <div className="bg-background p-5 rounded-lg border border-border shadow-sm flex flex-col items-center justify-center relative">
                ...
                <span
                  ...
                  title="Inversión en Retargeting y Automatizaciones"
                >
              ...
              <div className="bg-background p-5 rounded-lg border border-amber-500/20 shadow-sm flex flex-col items-center justify-center relative ring-1 ring-amber-500/10">
                <span className="text-[10px] text-amber-600 dark:text-amber-500 font-bold uppercase mb-1">
                  Interés Demostrado (MQL)
                </span>
                <span className="text-4xl font-black text-amber-600 dark:text-amber-500">
                  {formatNum(totalMqls)}
                </span>
                <span className="text-xs text-amber-600/80 dark:text-amber-500/80 mt-1 font-medium">
                  Leads Calificados (MQLs)
                </span>
              ...
              <div className="bg-orange-950 dark:bg-orange-950/50 p-5 rounded-lg border border-orange-900 shadow-md flex flex-col items-center justify-center relative text-white">
                <span className="text-[10px] text-orange-300 font-bold uppercase mb-1">
                  Listos para Comprar (SQL)
                </span>
                ...
                <span className="text-xs text-emerald-300 font-medium">
                  <Target className="w-3 h-3 mr-1 inline" /> Costo p/Oportunidad:{" "}
                  {fmtMoney(costPerSql)}
                </span>
              </div>
            </div>
            ...
            {activeBottlenecks.length > 0 && (
              <div className="flex flex-col gap-2 mb-4">
                {activeBottlenecks.map((bottleneck, idx) => (
                  <div
                    key={idx}
                    className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900 rounded-lg p-3 flex gap-3 text-red-900 dark:text-red-200 items-center shadow-sm"
                  >
                    <AlertTriangle className="w-5 h-5 text-red-500 flex-shrink-0" />
                    <div className="flex-1">
                      <p className="text-sm font-medium">
                        <strong>Cuello de Botella Detectado:</strong> {bottleneck.metricLabel} está
                        en {bottleneck.currentRate}%. {bottleneck.tip}
                      </p>
                    </div>
                    <Button
                      variant="outline"
                      size="sm"
                      className="bg-red-100 hover:bg-red-200 text-red-700 border-red-200 h-8"
                    >
                      Solucionar
                    </Button>
                  </div>
            ...
            <div className="bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-950/20 dark:to-orange-950/20 border border-amber-100 dark:border-amber-900/50 rounded-lg p-3 flex gap-3 text-amber-900 dark:text-amber-200 items-center shadow-sm">
              ...
              <p className="text-sm font-medium">
                <strong>Entiende tus números:</strong> <em>Nutrición</em> mide cuántos leads
                calentaste (MQLs) y <em>Oportunidad</em> cuántos de esos están a un paso de pagar o
                agendar (SQLs). Haz clic en los números para accionar.
              </p>
            </div>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        <div className="space-y-6">
          <div className="border-b border-border pb-2">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Canales que educan y convencen al lead de que tú eres la solución.
            </p>
          </div>
          ...
          <div className="bg-card rounded-lg border border-border shadow-sm overflow-hidden">
            <div className="bg-muted/50 px-4 py-3 border-b border-border flex justify-between items-center">
              <span className="font-medium text-sm text-foreground/90 flex items-center">
                <Workflow className="w-4 h-4 text-amber-600 mr-2" /> Automatizaciones & Secuencias
              </span>
              <div className="flex gap-4 text-xs font-medium">
                <span className="text-muted-foreground">
                  {formatNum(nurtureData.automation.totals.leads || 0)} MQLs
                </span>
              </div>
            </div>
                ...
                {splitChannels(nurtureChannels).rows.map((c) => {
                  ...
                  return (
                    <NurtureChannelRow
                      ...
                      defaultMetricLabel={metricLabel}
                      baseColor="amber"
                    />
                  );
                })}
          ...
          <div className="bg-card rounded-lg border border-border shadow-sm overflow-hidden">
            <div className="bg-muted/50 px-4 py-3 border-b border-border flex justify-between items-center">
              <span className="font-medium text-sm text-foreground/90 flex items-center">
                <Repeat className="w-4 h-4 text-blue-600 mr-2" /> Retargeting Omnicanal
              </span>
              <div className="flex gap-4 text-xs font-medium">
                <span className="text-muted-foreground">
                  {formatNum(nurtureData.retargeting.totals.leads || 0)} MQLs
                </span>
                <span className="text-red-500">
                  {fmtMoney(nurtureData.retargeting.totals.spend || 0)}
                </span>
              </div>
            </div>
            <div className="p-4 space-y-2">
              <div className="grid grid-cols-1 gap-2">
                {splitChannels(retargetingChannels).rows.map((c) => {
                  const isMeta =
                    c.slug.includes("meta") || c.slug.includes("fb") || c.slug.includes("ig");
        ...
        <div className="space-y-6">
          <div className="border-b border-border pb-2">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Donde el lead demuestra clara intención de pagar o hablar contigo.
            </p>
          </div>
          ...
          <div className="bg-card rounded-lg border border-border shadow-sm overflow-hidden border-l-4 border-l-orange-400 hover:border-l-orange-500 transition-colors">
            <div className="bg-orange-500/5 px-4 py-3 border-b border-border flex justify-between items-center">
              <span className="font-medium text-sm flex items-center text-foreground/90">
                <CalendarCheck className="w-4 h-4 text-orange-600 mr-2" /> Calificación & Citas
              </span>
              <div className="flex gap-4 text-xs font-medium">
                <span className="text-orange-600 font-bold">
                  {formatNum(oppData.qualification.totals.sqls || 0)} SQLs
                </span>
              </div>
            </div>
            <div className="p-4 space-y-2">
              <div className="grid grid-cols-1 gap-2">
                {splitChannels(oppQualificationChannels).rows.map((c) => (
                  <NurtureChannelRow
                    ...
                    channel={c}
                    stageId="OPORTUNIDAD"
                    defaultMetricName="sqls"
                    defaultMetricLabel="formularios / citas"
                    baseColor="orange"
                  />
                ))}
              </div>
          ...
          <div className="bg-card rounded-lg border border-border shadow-sm overflow-hidden border-l-4 border-l-emerald-500 hover:border-l-emerald-600 transition-colors">
            <div className="bg-emerald-500/5 px-4 py-3 border-b border-border flex justify-between items-center">
              <span className="font-medium text-sm flex items-center text-foreground/90">
                <ShoppingCart className="w-4 h-4 text-emerald-600 mr-2" /> Checkouts & Pagos
              </span>
              <div className="flex gap-4 text-xs font-medium">
                <span className="text-emerald-600 font-bold">
                  {formatNum(
                    (oppData.checkout.totals.sqls || 0) + (oppData.paymentLinks.totals.sqls || 0),
                  )}{" "}
                  SQLs
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2">
                {splitChannels(oppCheckoutChannels).rows.map((c) => (
                  <NurtureChannelRow
                    ...
                    stageId="OPORTUNIDAD"
                    defaultMetricName="sqls"
                    defaultMetricLabel="iniciaron pago"
                    baseColor="emerald"
                  />
                ))}
              </div>
