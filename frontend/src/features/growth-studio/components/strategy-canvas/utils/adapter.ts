...
export interface VisxLink
  extends
    Omit<MarketingActionLink, "source" | "target">,
    Omit<SankeyLink<MarketingNode, MarketingActionLink>, "source" | "target"> {
  source: number;
  target: number;
...
export const adaptStrategyToVisx = (config: StrategyCanvasConfig): VisxGraphData => {
  ...
  const sortedNodes = config.nodes
    .map((node) => ({ ...node }))
