...
describe("stageOverviewApi", () => {
    ...
    mockFetch.mockResolvedValue({
      ...
      json: () =>
        Promise.resolve({
          ...
          groups: [{ group_key: "paid", group_label: "Paid", channel_count: 2 }],
          channel_list: [
            {
              slug: "meta-ads",
              name: "Meta Ads",
  ...
  it("throws on non-ok response", async () => {
    ...
    await expect(stageOverviewApi.getStageOverview("token", "attraction")).rejects.toThrow(
      "Stage overview API returned 500",
    );
  });
});
