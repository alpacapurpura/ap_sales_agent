...
import { Loader2, CheckCircle, Calendar, Trash2, ExternalLink, Copy, Link as LinkIcon, Activity } from "lucide-react";
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Vincula tu cuenta de Google Workspace para gestionar citas y disponibilidad automáticamente.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
            <Alert className="bg-blue-50 text-blue-800 border-blue-200">
                ...
                <AlertDescription>
                    Actualmente solo soportamos cuentas de Google Workspace (Empresariales). Asegúrate de tener permisos para instalar aplicaciones.
                </AlertDescription>
            </Alert>
        ...
        <CardFooter>
            <Button onClick={handleConnect} disabled={connecting} className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700">
                {connecting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Calendar className="mr-2 h-4 w-4" />}
                Conectar con Google
            </Button>
        </CardFooter>
      </Card>
            ...
            <CardContent className="space-y-6">
                        ...
                        <AlertDescription>
                            ...
                            {testResult.data && (
                                <div className="mt-2 text-xs bg-background/50 p-2 rounded overflow-x-auto text-foreground border border-border/50 grid gap-1">
                                    ...
                                    <div><strong>Busy Slots (24h):</strong> {testResult.data.busy_slots}</div>
                                </div>
                            )}
                        </AlertDescription>
                    </Alert>
                ...
                <div className="pt-4 border-t space-y-4">
                    <div className="flex items-center justify-between">
                        <div className="space-y-1">
                             <span className="text-muted-foreground text-xs uppercase tracking-wider">Enlace de Reservas Público</span>
                             <p className="text-sm text-muted-foreground">Comparte este enlace para que tus leads agenden citas automáticamente.</p>
                        </div>
                        {!status.booking_link && (
                            <Button onClick={handleGenerateLink} disabled={generatingLink} size="sm" variant="outline">
                    ...
                    {status.booking_link && (
                        <div className="flex items-center gap-2">
                            <div className="bg-muted p-2 rounded text-sm font-mono flex-1 overflow-hidden text-ellipsis whitespace-nowrap border">
                                {typeof window !== 'undefined' ? window.location.origin : ''}{status.booking_link}
                            </div>
                            <Button 
                                size="icon" 
                                variant="secondary" 
                ...
                <Dialog>
                    ...
                    <DialogContent>
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                El sistema dejará de poder verificar tu disponibilidad y agendar nuevas citas. Las citas existentes no se borrarán de tu calendario.
                            </DialogDescription>
                        </DialogHeader>
                        <DialogFooter>
