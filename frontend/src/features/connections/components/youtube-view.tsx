...
import { Loader2, CheckCircle, Youtube, Trash2, ExternalLink, Activity, Settings } from "lucide-react";
  ...
  useEffect(() => {
    const handleMessage = async (event: MessageEvent) => {
      ...
      if (event.data?.type === "GOOGLE_OAUTH_SUCCESS" && event.data?.code) {
         try {
          // Only proceed if we initiated the connection (simple check: if we are mounted and this fires, likely intended for us if we are the active tab)
          // Ideally we would check state, but for now we rely on tab isolation.
          setConnecting(true);
          const token = await getToken();
          if (!token) return;
          ...
          await connectionsApi.connectYoutube(event.data.code, token, redirectUri);
                    ...
                    <form onSubmit={handleSaveConfig} className="space-y-4">
                        <Alert className="bg-amber-50 text-amber-800 border-amber-200">
                            ...
                            <AlertDescription className="text-xs">
                                1. Crea un proyecto en <a href="https://console.cloud.google.com/" target="_blank" className="underline font-medium">Google Cloud Console</a>.
                                <br />
                        ...
                        <div className="flex justify-end gap-2 pt-2">
                             ...
                             <Button type="submit" disabled={savingConfig}>
                                {savingConfig ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Guardar Configuración"}
                             </Button>
                        </div>
                    </form>
                </CardContent>
    ...
    return (
      <Card>
        ...
        <CardFooter className="flex justify-between">
            ...
            <Button onClick={handleConnect} disabled={connecting} className="bg-red-600 hover:bg-red-700">
                {connecting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Youtube className="mr-2 h-4 w-4" />}
    ...
    <div className="space-y-6">
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    Tu canal de YouTube está conectado y sincronizado.
                </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
                <div className="space-y-1">
                    <span className="text-muted-foreground text-xs uppercase tracking-wider">Canal Conectado</span>
                        ...
                        <AlertDescription>
                            ...
                            {testResult.data && (
                                <div className="mt-2 text-xs bg-background/50 p-2 rounded overflow-x-auto text-foreground border border-border/50 grid gap-1">
                                    <div><strong>Título:</strong> {testResult.data.title}</div>
                                    <div><strong>Vistas:</strong> {testResult.data.statistics?.viewCount}</div>
                                    <div><strong>Suscriptores:</strong> {testResult.data.statistics?.subscriberCount}</div>
                                </div>
                            )}
                        </AlertDescription>
                    </Alert>
