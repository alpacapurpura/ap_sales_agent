...
import { TierComparisonBuilder } from "./TierComparisonBuilder";
    ...
    <Card className="overflow-hidden border-slate-200 dark:border-slate-800">
      <CardHeader className="pb-4 border-b bg-slate-50 dark:bg-slate-900/50 flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>
            Configura tus planes de pago, suscripciones o precios únicos.
          </CardDescription>
        </div>
        ...
        {pricingFields.length === 0 ? (
          ...
          <div className="divide-y">
            {pricingFields.map((field, index) => {
              ...
              const installments =
                form.watch(`pricing_options.${index}.number_of_installments`) || 1;
              ...
              const calculatedInstallment =
                installments > 0 ? (totalAmount - deposit) / installments : 0;
                ...
                >
                    ...
                    <div className="flex items-center gap-4 flex-1">
                      <div
                        className={cn(
                          ...
                          isDefault
                            ? "bg-yellow-100 text-yellow-600 ring-2 ring-yellow-400 ring-offset-2"
                      ...
                      <div className="space-y-1 flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {isDefault && (
                            <Badge
                              variant="secondary"
                              className="bg-yellow-100 text-yellow-800 hover:bg-yellow-100 text-[10px] uppercase font-bold tracking-wider"
                            >
                              Popular
                            </Badge>
                          )}
                        ...
                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                          ...
                          {planType === PaymentPlanType.PAYMENT_PLAN && (
                            <span>
                              ...
                              {formatMoney(calculatedInstallment, currency)}
                            </span>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="pl-11">
                    <Tabs
                      ...
                      onValueChange={(val) =>
                        form.setValue(`pricing_options.${index}.plan_type`, val as PaymentPlanType)
                      }
                      className="w-full"
                    >
                      <TabsList className="mb-4 w-full justify-start h-auto p-1 bg-muted/50">
                        <TabsTrigger value={PaymentPlanType.ONE_TIME} className="text-xs">
                          Pago Único
                        </TabsTrigger>
                        <TabsTrigger value={PaymentPlanType.PAYMENT_PLAN} className="text-xs">
                          Plan de Pagos
                        </TabsTrigger>
                        <TabsTrigger value={PaymentPlanType.SUBSCRIPTION} className="text-xs">
                          Suscripción
                        </TabsTrigger>
                      </TabsList>
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-xs">
                                  Texto de Ahorro (Opcional)
                                </FormLabel>
                                <FormControl>
                                  <Input
                                    placeholder="Ej. Ahorra $200 vs Plan"
                                    {...field}
                                    value={field.value || ""}
                                  />
                                </FormControl>
                      ...
                      <TabsContent value={PaymentPlanType.PAYMENT_PLAN} className="mt-0 space-y-4">
                        ...
                        <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg flex items-center justify-between text-sm text-blue-700 dark:text-blue-300">
                          ...
                          <span>
                            Hoy paga <strong>{formatMoney(deposit, currency)}</strong> +{" "}
                            {installments} cuotas de{" "}
                            <strong>{formatMoney(calculatedInstallment, currency)}</strong>
                          </span>
                        </div>
                      </TabsContent>
                      ...
                      <TabsContent value={PaymentPlanType.SUBSCRIPTION} className="mt-0">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-1 text-xs text-muted-foreground pt-6">
                            La configuración de recurrencia se maneja en el procesador de pagos
                            (Stripe). Aquí solo defines el precio visual.
                          </div>
                        </div>
                      </TabsContent>
                    </Tabs>
