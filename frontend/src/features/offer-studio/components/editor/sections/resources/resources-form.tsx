...
function ResourcesContent({ form }: { form: UseFormReturn<OfferFormValues> }) {
  ...
  const multimediaFields = fields
    .map((f, i) => ({ ...f, index: i }))
    .filter((f) => !f.is_knowledge_base);
  const knowledgeFields = fields
    .map((f, i) => ({ ...f, index: i }))
    .filter((f) => f.is_knowledge_base);
            ...
            <CardContent className="pt-6 flex flex-col items-center justify-center text-center space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Fotos, Videos o Audios que el agente mostrará.
                </p>
              </div>
        ...
        <TabsContent value="knowledge" className="space-y-4 mt-4">
          <Card className="bg-slate-50 dark:bg-slate-900/50">
            <CardHeader>
              ...
              <CardDescription>
                Documentos técnicos que el agente leerá para responder preguntas específicas.
              </CardDescription>
            </CardHeader>
              ...
              <div className="space-y-2">
                ...
                {knowledgeFields.length === 0 && (
                  <p className="text-xs text-center text-muted-foreground italic py-2">
                    No hay documentos cargados. El agente usará solo su conocimiento general.
                  </p>
                )}
              </div>
            </CardContent>
...
export function ResourcesForm({ defaultValues: propValues, onSave }: ResourcesFormProps) {
  ...
  return (
    <SectionFormWrapper<ResourcesFormValues>
      ...
      onSubmit={handleSave}
    >
      {(form) => <ResourcesContent form={form as unknown as UseFormReturn<OfferFormValues>} />}
    </SectionFormWrapper>
  );
}
