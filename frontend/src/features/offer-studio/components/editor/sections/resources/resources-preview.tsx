...
export const ResourcesPreview = ({ data: propsData }: ResourcesPreviewProps) => {
  ...
  const resources = assets?.filter(a => 
  ...
  const getIcon = (type: string) => {
    switch (type) {
        case AssetType.PDF: return <FileText className="w-4 h-4 text-rose-500" />;
        case AssetType.TXT: return <FileText className="w-4 h-4 text-slate-500" />;
        case AssetType.AUDIO: return <Headphones className="w-4 h-4 text-violet-500" />;
        case AssetType.URL: return <LinkIcon className="w-4 h-4 text-sky-500" />;
  ...
  return (
    <div className="w-full grid grid-cols-1 gap-2">
      {resources.map((resource, idx) => (
        <div key={idx} className="flex items-center gap-3 p-3 rounded-lg border border-transparent hover:bg-muted/30 hover:border-border/40 transition-all group">
            ...
            <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-muted-foreground truncate opacity-70">{resource.type} • {resource.url}</p>
            </div>
