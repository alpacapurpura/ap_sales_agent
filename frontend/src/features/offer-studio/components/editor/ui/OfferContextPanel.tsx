...
export function getOfferValidationStatus(values: OfferFormValues): ValidationResult {
  const checks: ValidationItem[] = [
    // Phase 1: Context
    {
      ...
      valid: !!values.public_name && values.public_name.length > 3,
      phase: "context",
    },
    ...
    // Phase 2: Solution
    {
      label: "Gran Promesa (Headline)",
      valid: !!values.headline_promise && values.headline_promise.length > 10,
      phase: "solution",
    },
    ...
    {
      label: "Stack de Valor (Entregables)",
      valid: values.deliverables && values.deliverables.length > 0,
      phase: "solution",
    },
    ...
    // Date Checks for Events and Fixed Programs
    {
      ...
      valid: (() => {
        const details = values.specific_details as Record<string, unknown> | undefined;
        const isEvent = values.archetype === OfferArchetype.EXPERIENCIA;
        // Check string values of enum ProgramStructure
        const isFixedProgram =
          details?.structure_type === "FIXED_COHORT" || details?.structure_type === "CHALLENGE";
    ...
    // Phase 3: Deal
    {
      ...
      valid: values.pricing_options && values.pricing_options.length > 0,
      phase: "deal",
    },
    ...
    return (
      <Card className="bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-900 mb-6">
        ...
        <CardContent>
          ...
          <Progress
            value={100}
            className="h-2 mt-3 bg-emerald-200 dark:bg-emerald-900"
    ...
    <Card className="bg-amber-50/50 dark:bg-amber-950/10 border-amber-200/50 dark:border-amber-900/50 mb-6 shadow-sm transition-all duration-300">
      ...
      >
        ...
        <Progress
          ...
          className="h-1.5 mt-2 bg-amber-100 dark:bg-amber-900"
          ...
          <div className="pr-1 pt-2">
            <ul className="space-y-2">
              {status.missingItems.map((item, idx) => (
                <li
                  key={idx}
                  className="flex items-start gap-2 text-xs text-muted-foreground bg-background/50 p-2 rounded border border-transparent hover:border-amber-200 transition-colors cursor-help group"
                >
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500 pb-10">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Definir claramente <strong>quién compra</strong> y <strong>por qué</strong> es el 80% del
          éxito.
        </p>
      </div>
      ...
      {isKnowledgeOffer && values.instructors && values.instructors.length > 0 && (
        <InstructorsPreview data={{ instructors: values.instructors }} />
      )}
      ...
      <Card className="bg-background border-primary/20 shadow-sm overflow-hidden">
        ...
        <CardHeader className="pb-2">
          <div className="flex justify-between items-start">
            ...
            {avatar?.scope === "OFFER_SPECIFIC" && (
              <Badge className="text-[10px]">Específico</Badge>
            )}
          </div>
              ...
              <Sheet>
                ...
                <SheetContent className="w-[400px] sm:w-[540px] overflow-y-auto">
                  <SheetHeader>
                    ...
                    <SheetDescription>Perfil detallado del cliente ideal.</SheetDescription>
                  </SheetHeader>
                  ...
                  <div className="space-y-6 py-6">
                    ...
                    {avatar.voice_tone_config &&
                      Object.keys(avatar.voice_tone_config).length > 0 && (
                        <div className="space-y-2">
                          <h4 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">
                            Voz y Tono
                          </h4>
      ...
      <AlertDialog open={showSaveAlert} onOpenChange={setShowSaveAlert}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Has modificado la oferta actual. Si sales ahora para editar el avatar, podrías perder
              tu progreso. ¿Quieres guardar los cambios antes de ir al editor de avatar?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={handleConfirmSaveAndEdit}>
              Guardar y Editar
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      <div className="bg-blue-50 dark:bg-blue-950/20 p-4 rounded-lg border border-blue-100 dark:border-blue-900">
        ...
        <ul className="text-xs space-y-2 text-muted-foreground">
          <li className="flex gap-2">
            ...
            Usa palabras viscerales (&quot;Miedo a quebrar&quot; vs &quot;Problemas
            financieros&quot;).
          </li>
          <li className="flex gap-2">
            <CheckCircle2 className="w-3 h-3 mt-0.5 text-blue-500" />
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500 pb-10">
      ...
      <Card
        className={cn(
          ...
          !hasAssets
            ? "bg-slate-50 border-slate-300 dark:bg-slate-900/50"
            : isEquipped
      ...
      >
        <CardHeader className="pb-2">
          <div className="flex justify-between items-start">
            <CardTitle className="text-sm font-semibold flex items-center gap-2">
              <Zap
                className={cn(
                  "w-4 h-4",
                  isEquipped ? "text-indigo-600 fill-indigo-600" : "text-slate-400",
                )}
              />
          ...
          <CardDescription className="text-xs">
            ...
            {hasAssets &&
              !isEquipped &&
              "Mejorando... Sigue agregando recursos visuales y técnicos."}
        ...
        <CardContent className="space-y-3 text-xs">
          ...
          <div className="space-y-1">
            <div className="flex justify-between text-muted-foreground">
              <span className="flex items-center gap-1">
                <CheckCircle2
                  className={cn(
                    "w-3 h-3",
                    salesAssets.length > 0 ? "text-emerald-500" : "text-slate-300",
                  )}
                />{" "}
                Persuasión Visual
              </span>
            ...
            <Progress
              value={Math.min(salesAssets.length * 33, 100)}
              className="h-1.5"
              indicatorClassName={salesAssets.length > 0 ? "bg-emerald-500" : "bg-slate-200"}
            />
          </div>
          ...
          <div className="space-y-1">
            <div className="flex justify-between text-muted-foreground">
              <span className="flex items-center gap-1">
                <Book
                  className={cn(
                    "w-3 h-3",
                    knowledgeDocs.length > 0 ? "text-blue-500" : "text-slate-300",
                  )}
                />{" "}
            ...
            <Progress
              value={Math.min(knowledgeDocs.length * 50, 100)}
              className="h-1.5"
              indicatorClassName={knowledgeDocs.length > 0 ? "bg-blue-500" : "bg-slate-200"}
            />
          </div>
              ...
              <ul className="space-y-1">
                {values.deliverables.slice(0, 3).map((d, i) => (
                  <li key={i} className="text-xs flex justify-between">
                    ...
                    <span className="text-muted-foreground">
                      {formatMoney(d.value_stack_price || 0, displayCurrency, {
                        fractionDigits: 0,
                      })}
                    </span>
                  </li>
                ...
                {values.deliverables.length > 3 && (
                  <li className="text-[10px] text-muted-foreground italic">
                    + {values.deliverables.length - 3} más...
                  </li>
                )}
              </ul>
            </div>
              ...
              <div className="text-right">
                ...
                {mainPrice.number_of_installments > 1 && (
                  <span className="text-[10px] text-muted-foreground">
                    o {mainPrice.number_of_installments} pagos de{" "}
                    {formatMoney(mainPrice.installment_amount || 0, displayCurrency, {
                      fractionDigits: 0,
                    })}
                  </span>
                )}
            ...
            <div className="flex gap-2 items-start">
              ...
              <p className="text-xs text-muted-foreground text-left">
                ...
                <span className="font-medium text-foreground">
                  {values.guarantee_type || "Estándar"}
                </span>
              </p>
            </div>
            <div className="flex gap-2 items-start">
