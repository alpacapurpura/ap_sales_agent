    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between pb-2 border-b border-indigo-100 dark:border-indigo-900/30">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button 
            type="button" 
            size="sm" 
            onClick={() => append({ title: `Módulo ${fields.length + 1}`, description: "", topics: [] })}
      ...
      {fields.length === 0 ? (
        ...
        >
          ...
          <Button 
             variant="link" 
             onClick={(e) => { e.stopPropagation(); setIsImportDialogOpen(true); }}
        ...
        <Accordion type="single" collapsible className="w-full space-y-0 border rounded-lg divide-y bg-white dark:bg-slate-950">
...
function CurriculumModuleItem({ index, form, remove }: { index: number, form: UseFormReturn<OfferFormValues>, remove: (index: number) => void }) {
    // @ts-ignore
    const topics = form.watch(`specific_details.curriculum.${index}.topics`) || [];

    const addTopic = (e: React.MouseEvent) => {
        e.preventDefault(); 
        const current = form.getValues(`specific_details.curriculum.${index}.topics` as any) || [];
        form.setValue(`specific_details.curriculum.${index}.topics` as any, [...current, ""]);
    };

    const removeTopic = (tIndex: number) => {
        const current = form.getValues(`specific_details.curriculum.${index}.topics` as any) || [];
        form.setValue(`specific_details.curriculum.${index}.topics` as any, current.filter((_: any, i: number) => i !== tIndex));
    };
        ...
        <AccordionItem value={`item-${index}`} className="border-b-0 px-0">
            <div className="flex items-center gap-3 py-2 px-4 hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors group/item">
                ...
                <div className="flex-1">
                    <FormField
                        control={form.control}
                        name={`specific_details.curriculum.${index}.title` as any}
                        render={({ field }) => (
            ...
            <AccordionContent className="pt-0 pb-4 px-4 pl-11">
                <div className="space-y-4 pt-2">
                    ...
                    <FormField
                        control={form.control}
                        name={`specific_details.curriculum.${index}.description` as any}
                        render={({ field }) => (
                            <FormItem className="space-y-0">
                                <FormControl>
                    ...
                    <div className="space-y-1">
                        <div className="flex items-center justify-between mb-2">
                            <Label className="text-[10px] uppercase font-bold text-muted-foreground/70 tracking-wider">Lecciones</Label>
                            <Button 
                                type="button" 
                                variant="ghost" 
                                size="sm" 
                        ...
                        <div className="space-y-1">
                            {topics.map((_: any, tIndex: number) => (
                                <div key={tIndex} className="group/topic flex items-center gap-2">
                                    ...
                                    <FormField
                                        control={form.control}
                                        name={`specific_details.curriculum.${index}.topics.${tIndex}` as any}
                                        render={({ field }) => (
                                            <Input 
                                                {...field} 
