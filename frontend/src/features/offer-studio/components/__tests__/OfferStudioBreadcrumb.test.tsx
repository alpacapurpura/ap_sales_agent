...
describe("OfferStudioBreadcrumb · buildCrumbs", () => {
  ...
  it("returns the root crumb alone when tenantId is missing", () => {
    expect(
      buildCrumbs({
        pathname: "/t-1/offer-studio/offer/o-1/editor",
        tenantId: "",
        resolveSectionLabel: resolveLabel,
      }),
    ).toEqual([{ label: "Offer Studio", href: undefined }]);
  });
  ...
  it("appends the section label for editor sub-routes", () => {
    const crumbs = buildCrumbs({
      pathname: "/t-1/offer-studio/offer/o-1/editor/promise",
      tenantId: tenant,
      resolveSectionLabel: resolveLabel,
    });
  ...
  it("supports the legacy /edition/{code}/{section} URL shape", () => {
    const crumbs = buildCrumbs({
      pathname: "/t-1/offer-studio/offer/o-1/edition/default/pricing",
      tenantId: tenant,
      resolveSectionLabel: resolveLabel,
    });
  ...
  it("labels the editions tab and adds edition id crumb on detail routes", () => {
    const crumbs = buildCrumbs({
      pathname: "/t-1/offer-studio/offer/o-1/editions/ed-123",
      tenantId: tenant,
      resolveSectionLabel: resolveLabel,
    });
  ...
  it("labels known sibling tabs (Assets / Knowledge / Campaigns / Ventas)", () => {
    const assets = buildCrumbs({
      pathname: "/t-1/offer-studio/offer/o-1/assets",
      tenantId: tenant,
      resolveSectionLabel: resolveLabel,
    });
    expect(assets.at(-1)?.label).toBe("Assets");

    const ventas = buildCrumbs({
      pathname: "/t-1/offer-studio/offer/o-1/ventas",
      tenantId: tenant,
      resolveSectionLabel: resolveLabel,
    });
  ...
  it("falls back to the raw segment for unknown tabs", () => {
    const crumbs = buildCrumbs({
      pathname: "/t-1/offer-studio/offer/o-1/unknown-tab",
      tenantId: tenant,
      resolveSectionLabel: resolveLabel,
    });
