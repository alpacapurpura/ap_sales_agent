...
export function AvatarSelectionView({ offerId }: AvatarSelectionViewProps) {
  ...
  const selectAvatarMutation = useMutation({
    mutationFn: async (avatarId: string) => {
        const token = await getToken();
        if (!token) throw new Error("No token");
        return offerApi.saveOffer(offerId, { avatar_id: avatarId }, token);
    },
    onSuccess: () => {
  ...
  if (isLoading) {
    return (
        <OfferEditorLayout offerId={offerId}>
