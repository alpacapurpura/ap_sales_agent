...
const DELIVERY_BADGES = {
  [OfferDeliveryModel.DIY]: {
    label: "DIY",
    color: "bg-emerald-500/10 text-emerald-600 border-emerald-500/20",
  },
  [OfferDeliveryModel.DWY]: {
    label: "DWY",
    color: "bg-blue-500/10 text-blue-600 border-blue-500/20",
  },
  [OfferDeliveryModel.DFY]: {
    label: "DFY",
    color: "bg-purple-600/10 text-purple-600 border-purple-600/20",
  },
  [OfferDeliveryModel.HYBRID]: {
    label: "Hybrid",
...
export const OfferCard = memo(function OfferCard({
  ...
  const badgeConfig =
    DELIVERY_BADGES[offer.delivery_model] || DELIVERY_BADGES[OfferDeliveryModel.DIY];
      ...
      <div className={cn("flex flex-col h-full justify-between", compact ? "p-3" : "p-4")}>
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2 mb-1">
            <Badge
              variant="outline"
              className={cn("text-[10px] h-5 px-1.5 py-0 font-bold border", badgeConfig.color)}
            >
              <HighlightedText text={badgeConfig.label} query={searchQuery} />
            </Badge>
            <Badge
              variant="secondary"
              className="text-[10px] h-5 px-1.5 py-0 font-medium bg-muted text-muted-foreground border-transparent"
            >
