...
const ARCHETYPE_COLORS: Record<string, string> = {
  producto:
    "bg-blue-100 text-blue-700 border-r border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800",
  programa:
    "bg-orange-100 text-orange-700 border-r border-orange-200 dark:bg-orange-900/30 dark:text-orange-400 dark:border-orange-800",
  servicio:
    "bg-violet-100 text-violet-700 border-r border-violet-200 dark:bg-violet-900/30 dark:text-violet-400 dark:border-violet-800",
  membresia:
    "bg-emerald-100 text-emerald-700 border-r border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800",
  experiencia:
    "bg-amber-100 text-amber-700 border-r border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800",
  default:
  ...
  const priceDisplay =
    offer.pricing && offer.pricing.length > 0
      ? new Intl.NumberFormat("en-US", {
        ...
        }).format(offer.pricing[0].total_amount)
      : "Gratis";
    ...
    >
      ...
      <div className="flex-1 px-3 py-2 min-w-0 flex flex-col justify-center h-full">
        <div className="flex items-center justify-between gap-2 mb-0.5">
          <Badge
            variant="secondary"
            className="text-[9px] h-4 px-1 rounded-sm font-normal bg-muted text-muted-foreground border-transparent truncate max-w-[120px]"
          >
            {typeLabel}
          </Badge>
        ...
        <h4
          className="font-semibold text-xs leading-tight line-clamp-2 text-foreground/90 group-hover:text-primary transition-colors"
          title={offer.name}
        >
          {offer.name}
        </h4>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            {offer.landing_page_config?.is_published && (
              <DropdownMenuItem
                onClick={() => window.open(`/p/${offer.landing_page_config?.slug}`, "_blank")}
              >
      ...
      <AlertDialog open={showArchiveDialog} onOpenChange={setShowArchiveDialog}>
        <AlertDialogContent onClick={(e) => e.stopPropagation()}>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              La oferta se ocultará de la lista principal y su landing page quedará despublicada.
              Podrás restaurarla desde la vista de archivados.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
