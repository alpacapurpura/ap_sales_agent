...
export function AttractionCaptureDetail({ onMetricClick, onConfigure }: AttractionCaptureDetailProps) {
  ...
  const totalReach = attrGroups.reduce((sum, g) => sum + (g.totals.reach ?? 0), 0);
  const totalVisitors = attrGroups.reduce((sum, g) => sum + (g.totals.visitors ?? g.totals.sessions ?? 0), 0); // Using sessions from GA4 / Visitantes from GA4
  const totalSpend = attrGroups.reduce((sum, g) => sum + (g.totals.spend ?? 0), 0);

  // Capture totals
  const totalLeads = capData.headerKpis.totalLeads || 0;
  const leadConvRate = totalVisitors > 0 ? (totalLeads / totalVisitors) * 100 : 0;
  ...
  const handleMetricClick = (stageId: 'ATRACCION' | 'CAPTURA', channel: any, metricName: string, val: number) => {
    if (onMetricClick) {
      onMetricClick({ 
        stageId, 
