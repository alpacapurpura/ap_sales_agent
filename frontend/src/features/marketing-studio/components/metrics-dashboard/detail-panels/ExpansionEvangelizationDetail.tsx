...
export function ExpansionEvangelizationDetail({ onMetricClick, onConfigure }: ExpansionEvangelizationDetailProps) {
  ...
  // --- KPIs Globales ---
  // Expansion
  const avgLtv = expData.headerKpis.avgLtv || 0;
  const actives = expData.miniFunnel.sourceValue || 0;
  const netMrr = expData.headerKpis.netMrr || 0;
  const expansionCount = expData.miniFunnel.targetValue || 0;
  const expansionRate = expData.miniFunnel.conversionRate || 0;
  const churnRate = expData.headerKpis.churnRatePct || 0;
  
  // Evangelization
  const kFactor = evaData.headerKpis.kFactor || 0;
  const referralConversions = evaData.headerKpis.referralConversions || 0;
  const npsScore = evaData.headerKpis.npsScore;
  const referralRevenue = evaData.headerKpis.referralRevenue || 0;

  const handleMetricClick = (stageId: 'EXPANSION' | 'EVANGELIZACION', channelSlug: string, metricName: string, val: number, channelName?: string) => {
    if (onMetricClick) {
      onMetricClick({ 
        stageId, 
        channelSlug, 
