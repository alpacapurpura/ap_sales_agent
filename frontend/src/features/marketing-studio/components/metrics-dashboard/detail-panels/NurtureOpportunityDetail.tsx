...
export function NurtureOpportunityDetail({ onMetricClick, onConfigure }: NurtureOpportunityDetailProps) {
  ...
  // Total active leads in nurture
  const totalNurtureLeads = 
  ...
  // Total MQLs
  const totalMqls = nurtureData.headerKpis.totalMqls || 0;
  
  // Total spend in nurture
  const totalNurtureSpend = nurtureData.retargeting.totals.spend || 0;

  // Opportunity KPIs
  const totalSqls = oppData.headerKpis.totalSqls || 0;
  
  // Conversion Rate MQL -> SQL
  const oppConvRate = totalMqls > 0 ? (totalSqls / totalMqls) * 100 : 0;
  
  // Cost per SQL (total spend / SQLs)
  const costPerSql = totalSqls > 0 ? totalNurtureSpend / totalSqls : 0;

  // Helper to get specific metric
  const getMetric = (metrics: any[], name: string) => metrics.find((m) => m.name === name)?.value ?? 0;

  // Organize channels
  const nurtureChannels = nurtureData.automation.channels.map(c => {
  ...
  const retargetingChannels = nurtureData.retargeting.channels.map(c => {
  ...
  const oppQualificationChannels = [
    ...(oppData.qualification.channels || []),
  ];
  
  const oppCheckoutChannels = [
  ...
  const activeBottlenecks = oppData.bottlenecks.filter(b => b.severity !== 'normal');

  const handleMetricClick = (stageId: 'NUTRICION' | 'OPORTUNIDAD', channel: any, metricName: string, val: number) => {
    if (onMetricClick) {
      onMetricClick({ 
        stageId, 
