...
export function PersonalityCloneForm() {
  ...
  const handleAnalyze = async () => {
    if (!userId) {
      setError("Please enter a User ID (Simulated Auth)")
      return
    }
    if (!textInput && !file) {
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-0.5">
        <h3 className="text-lg font-medium">Clonación de Estilo</h3>
        <p className="text-sm text-muted-foreground">
          Sube tus mejores chats para que Visionaria aprenda a hablar exactamente como tú.
        </p>
      </div>
      ...
      {step === "upload" && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Pega un historial de chat (WhatsApp/Email) o sube un archivo .txt exportado.
            </CardDescription>
          </CardHeader>
