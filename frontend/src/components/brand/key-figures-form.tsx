...
interface KeyFiguresFormProps {
    initialData: KeyFigure[];
    onSave: (team: KeyFigure[]) => Promise<void>;
    isSaving: boolean;
}

export function KeyFiguresForm({ initialData, onSave, isSaving }: KeyFiguresFormProps) {
        ...
        <div className="space-y-8">
            ...
            <Card className="w-full min-h-[600px]">
                ...
                <CardContent className="grid gap-4 md:grid-cols-2">
                    {team.map(member => (
                        <div key={member.id} className="flex items-start justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors">
                            ...
                            <div className="flex space-x-2">
                                <Button variant="ghost" size="icon" onClick={() => openTeamModal(member)}>
                                    <Edit2 className="h-4 w-4" />
                                </Button>
                                <Button variant="ghost" size="icon" className="text-destructive hover:text-destructive" onClick={() => deleteTeamMember(member.id)}>
