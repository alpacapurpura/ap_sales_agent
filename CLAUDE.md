...
11. **Spanish text (neutro LatAm, sin voseo):** Todo texto visible al usuario DEBE estar en **español latinoamericano neutro** (tuteo `tú`, nunca voseo `vos/tenés/podés/querés/mirá/dejá/poné/seleccioná/dale`). Nicolify se vende en todo Latam — un dejo argentino/rioplatense excluye MX/CO/PE/CL. También: tildes y eñes correctas (`días`, `Campaña`, `Inversión`). Aplica a componentes, schemas (labels/hints/placeholders), catálogos backend user-facing, prompts LLM de output visible, emails y notificaciones. Verificar antes de commitear. See `.claude/rules/spanish-text.md`.
