...
| **S11A** | [shared base lift](phases/S11-shared-lift-orchestrator-decomp.md) (sub-fase A) | 🚧 IN_PROGRESS | `BaseAgentCallbackHandler` lift coordinado (sales + copilot retrofit). Snapshot pipeline framework determinístico. |
| **S11B** | [orchestrator decomposition](phases/S11-shared-lift-orchestrator-decomp.md) (sub-fase B) | 📋 PLANNED | Strangler Fig refactor `chat.py` (1140→<400 LOC) + `closer_studio_service.py` (Query/Command/Kpi split) + `semantic_router.py` (registry). NO toca §3. |
