...
| **S6.5** | [legacy drop + admin cutover](phases/S6-5-legacy-drop-admin-cutover.md) | 📋 PLANNED | Drop tablas legacy `agent_trace_model` + `LLMLogModel` (post 2026-05-26). Cutover `sales_audit.py`. Arch tests `test_no_legacy_agent_trace_reads.py` + `test_admin_no_legacy_table_reads.py` + `test_no_future_annotations_in_langgraph_files.py`. Cleanup docs `agent_log_model` ghost name. |
...
| S10 | [quality eval loop](phases/S10-quality-eval-loop.md) | 📋 PLANNED | Judge multi-rubric + goldens + weekly cron + DeepSeek alias retire validator + Kimi temp 0.6 conversion monitor |
| **S11** | [shared base lift + orchestrator decomposition](phases/S11-shared-lift-orchestrator-decomp.md) | 📋 PLANNED | `BaseAgentCallbackHandler` lift coordinado (sales + copilot retrofit). Stranger Fig refactor `chat.py` (1082→<400 LOC) + `closer_studio_service.py` (Query/Command/Kpi split) + `semantic_router.py` (registry). NO toca §3. |
