...
| **S12** | [final hardening — zero debt](phases/S12-final-hardening-zero-debt.md) | ✅ DONE | Tier pricing >200k arch ratchet + tier resolution opt-in (`raw_payload` JSONB) + `typing_simulation_cpm` wiring per-canal (§3 calibration preservada) + DeepSeek alias retire arch ratchet (`deepseek-chat`/`deepseek-reasoner` post 2026-07-24 bloqueados) + Streamlit `/sales-routing` admin page (Kimi conversion monitor mínimo viable) + Presidio WONT-FIX documentado + scan voseo final (0 hits actionables) + skill `sales-agent-expert` post-redesign-aware (≤8000 chars) + audit `05-tech-debt-log.md` cero DEFERRED flotantes. **Plan cerrado.** 645 arch + 3113 modules + 95 quality/snapshot verde. §3 intacto. |
