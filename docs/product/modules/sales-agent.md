...
| **Goldens dataset infra (Story D)** | **done (merged 2026-05-08)** | Story D `sales-agent-goldens-3-tenants-dataset` (2026-05-08, 5 tickets, 262 tests + 1016 arch fitness). Eval suite ground truth = 20-30 goldens curados manualmente por Chris desde simulación dual-LLM (Story C personas + Story B simulator) post-merge. Generación via `backend/scripts/generate_golden_candidates.py` → curación Markdown preview (D12) → `backend/scripts/promote_golden.py` deterministic safe_dump. PII defense-in-depth via pre-commit hook Section 9 + `test_goldens_no_committed_pii` arch gate (NO whitelist policy D10). Schema `_schema.py` Pydantic v2 frozen `schema_version: Literal[1]` cement; parallel migrations registry to simulator (no mirror). Coverage gate ≥1 per 15-cell matrix (5 tenants × 3 persona_kinds happy/nurture/unqualified — adversarial out of scope, Story I). Cost-bucket invariant arch gate env-gated. T-3.bis follow-ups CLOSED 2026-05-08: DEEPER-A (commit 21f2f641 agent_bridge ContextVar session injection), DEEPER-B (commit b28c28ea archetype_slug propagation), DEEPER-C (commit 334be34e synthetic lead seed for FK), DEEPER-D (commit 3733fb2c promote_golden refuses crashed simulations + normalize-case). 3 RUN_EVALS env-gated smokes verified GREEN end-to-end with real LLM (TestReproducibilitySmoke + TestE2ESmoke + cost_bucket_invariant). Goldens curation manual paradigm post-merge by Chris. |
| **MAJ-EVAL grader runtime (Story E)** | **developed (pending audit)** | Story E `sales-agent-voice-fidelity-grader-runtime` (2026-05-09, 10 tickets, 158 ticket tests + 1063 arch fitness). Eval suite incluye runtime grader MAJ-EVAL multi-judge debate (3 judges heterogéneos Sonnet/GPT-4o/Kimi con weighted aggregation 0.4/0.4/0.2 + Round 2 peer critique on variance >0.15). 4 rubrics in scope (voice-fidelity + qualification-accuracy NEW Story E owns v1 + no-overpromise + no-hallucination). Cost-bucket separation cement: judge LLM calls escriben `eval_simulator_llm_call` ÚNICAMENTE — cero contamination producción. Sandbox markers `<<TRANSCRIPT_BEGIN>>...<<TRANSCRIPT_END>>` en Slot 5 + system directive Slot 1 (DQ2 anti-injection defense-in-depth). Cache hash-based (5 fields: transcript + rubric_id + voice + judge_set + version) en tabla `eval_simulator_grade_cache` — target ≥70% hit en re-runs idempotentes. Public API surface expandida 7→8 nombres: `grade_transcript_maj_eval` agregado (H9 cement). Upstream dependencies: Stories B (simulator harness) + C (personas archetype-aware) + D (goldens ground truth). Downstream consumers: Stories F (pass^k tracking) + G (CI gate enforcement) + I (adversarial jailbreak suite full). |
