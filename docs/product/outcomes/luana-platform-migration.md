...
stories_active: [luana-vitalia-bootstrap]   # 2026-05-13 Sesion 2 close — Story 11 state=ready (02-design-agentic + 03-arch consolidado + per-surface BE/FE/AGENTIC + 04-validators 70 + 05-guidelines + 06-tickets 38 atomic); Sesion 3 awaits /dev-team autonomous build pickup
...
| 9 | **40 sales_agent pre-existing failures + expanded scope Sesión 8** | **B — Continue defer Story 14 brand-voice-elevation natural home (EXPANDED 2026-05-14)** | Story 14 explicitly toca sales_agent surface (PersonalityProfile + voice cloning refactor). `docs/product/stories/luana-nicolify-migration/DEFERRED-FAILURES-STORY-10.md` (created Sesión 8 Option C ratification) lista 14 fallas live post-Fase-3 (6 baseline + 8 NEW) — TODAS sales_agent/eval framework/grader. Auditor verifica delta=0 NEW NOT-deferred (Option C HYBRID Sesión 8 fixed 13 NOT-deferred via codemod aug + direct test edits). Story 14 hereda total ~54 deferred = 40 original + 14 documented Sesión 8. **CRITICAL:** failure #14 (`test_sales_agent_tenant_isolation::test_every_observability_query_filters_tenant_id`) — security R2 violation en `dual_write_reconciliation_task.py:75` — considerar hot-fix Sesión 9 antes Story 14 timeline. |
...
12. **Sub-agent durante T-2..T-7 codemod application encounters SQLAlchemy runtime collision** (`InvalidRequestError: Table 'X' is already defined`, `NoReferencedTableError: Foreign key ... could not find table`, `SAWarning declarative base already contains class`) OR equivalent runtime singleton conflict surfaced at `pytest --collect-only`. Halt-and-ask Chris entre: (A) Pattern P6 shared-base lift first via T-2-prep (recommended default — if not already applied, verify with `python -c "from src.shared.domain.base_entity import Base as A; from luana_core_platform.domain.base_entity import Base as L; assert A is L"`), (B) Pattern P2 transitive stub closure (per-module — fragile), (C) Pattern P3 dual-Base isolation (REJECTED by anti-duplication §0 — only choose with explicit Chris approval). **Ratified Chris 2026-05-13 Session 6 Phase D + addendum `03-arch-be-addendum-2026-05-13.md` §6.1.** Inheritance §7.6.1: applies forward to Stories 11-14 lift challenges by default.
