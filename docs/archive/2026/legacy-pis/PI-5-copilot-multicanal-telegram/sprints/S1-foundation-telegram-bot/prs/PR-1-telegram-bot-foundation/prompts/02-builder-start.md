...
Surface a implementar: {agentic | frontend}  ← elegir según builder. PR-1 = cross-scope agentic + frontend (paralelo).
PR folder: /home/chris/AISALESHT/docs/pm-nico/pis/active/PI-5-copilot-multicanal-telegram/sprints/S1-foundation-telegram-bot/prs/PR-1-telegram-bot-foundation
Modules touched (agentic builder): copilot/api/, copilot/application/, copilot/infrastructure/, copilot/domain/ (si decides domain ports), shared/agent_observability/channels/format.py (READ ONLY reuse), core/config.py (ADD env var COPILOT_TELEGRAM_BOT_TOKEN). Migration en backend/alembic/versions/.
Modules touched (frontend builder): frontend/src/app/(main)/[tenantId]/(dashboard)/settings/copilot/telegram/, frontend/src/features/copilot/ (extender con hook + types Zod).
Iter actual: 1
Cross-scope notes:
- 2 builders paralelo: agentic Opus + frontend Sonnet. Cada uno spawnea SU auditor.
- Coordinación: commits por nombre archivo. IMPL-LOG.md sección "agentic" + sección "frontend" separadas.
- Builder agentic NO toca frontend/. Builder frontend NO toca backend/.
- Auditores: nicolify-agentic-auditor (Opus) + nicolify-frontend-auditor (Opus). Ambos PASS antes /pm cerrar.

PR-1 specific reminders:
- D-PI5-005 separación física: NO importar de connections/infrastructure/channels/. Solo READ como referencia patrón.
- D-PI5-026 webhook NON-BLOCKING: handler encola ARQ, devuelve 200 < 200ms. Inline LLM = anti-pattern A8.
- D-PI5-029 filter chat.type='private': anti-pattern A11.
- D-PI5-028 X-Telegram-Bot-Api-Secret-Token validation: anti-pattern A10 (return 401 strict).
- D-PI5-019 magic link: HMAC-SHA256, TTL 15 min, single-use, hash en DB (no plaintext).
- D-PI5-030 sanitize_payload antes persist: NO `username`/`first_name`/`phone` en logs (anti-pattern A3).
- Arch fitness tests obligatorios: test_copilot_telegram_bot_token_is_global, test_copilot_channel_links_no_cross_module_fk.
- Memory/cache config Telegram-specific (D-PI5-006..009) = OUT OF SCOPE PR-1, S2 PR-2. MVP usa default config web.
- HITL escalation (D-PI5-010..014) = OUT OF SCOPE PR-1, S3 PR-3.
- Push notifs proactivas = OUT OF SCOPE PR-1, S4 PR-4.
```
