...
|---|---|---|---|
| Q1 | Rate limit `/voice/transcribe` legacy | **BE deprecated 410 Gone (este PR). FE migration → PR follow-up cross-stack.** Decisión original era eliminar completo; bloqueador real: `frontend/src/features/copilot/api/voice-api.ts` aún llama el endpoint. Endpoint retorna 410 Gone con header `X-Deprecation-Notice`. FE migration queda como PR follow-up (PI-2 S1 PR-2 o similar). Reconocido en auto-fix iter 1 (ver IMPL-LOG). | §4 (rutas), §7 (flow) |
