...
## Sprint 6.A-D (offer-studio catalog + schemas + catch-all route)

- **Backend catalog as 5th SSoT axis needs two arch tests, not one.** Pattern locked for offer-studio: `test_section_catalog_completeness.py` (enum ↔ catalog alignment) + `test_archetype_sections_alignment.py` (cross-catalog scope invariants). Keeping them split makes failures actionable — the first says "add a metadata row", the second says "an archetype lists a section with wrong scope". One merged test obscures which rule broke.
- **`@vitest-environment` docblock breaks the pool.** Happy-dom is configured globally in `vitest.config.mts`. Adding the docblock directive to individual files caused "Cannot find package 'happy-dom'" because the fork looked for the package via a different resolution root. Removed it; the global config is sufficient. Pre-existing `use-auto-save` failures documented separately in `docs/mejoras-proceso/to-do.md`.
- **Working directory drift in long sessions.** After many Bash commands, `pwd` sometimes reports `/home/chris/aisalesht` (wrong case) instead of `/home/chris/AISALESHT/frontend`. Always `cd` explicitly at the start of each multi-command block; don't rely on session persistence for CWD.
- **Tests outside `tests/` folder hit alias resolution issues.** Hooks under `__tests__/` subfolders fail to resolve `@/lib/config` when the test transitively imports a module depending on it. Fix: place new tests in `features/offer-studio/tests/` where alias resolution works, AND mock the transitive catalog chain (`vi.mock("../hooks/use-archetype-catalog", ...)`).
- **ESLint `prefer-regexp-exec` clashes with security hooks.** Rewriting `.match()` in a way that invokes the alternative method triggered a hook blocking shell-command patterns. Resolution: keep `.match()` with an inline `eslint-disable-next-line` comment justifying the choice. The rule is optional; both forms are semantically equivalent for capture-group extraction.
- **Route catch-all with virtual edition code works with reserved keyword.** D19 locks `/offer-studio/offer/{offerId}/edition/{code}/{section}/{fieldId?}`. `code = "evergreen"` is treated as a reserved string by `useOfferEditionResolver` — no DB row, pure-function resolution, hides EDITION_LEVEL sections. This keeps the URL shape uniform across archetypes without needing a new kind enum migration backend-side.
