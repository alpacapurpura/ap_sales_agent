"""add_personality_profiles_table.
...
def upgrade() -> None:
    """Create personality_profiles table with indexes."""
    op.execute("""
    ...
    op.execute(
        ...
        "AND offer_id IS NULL AND avatar_id IS NULL"
    )


def downgrade() -> None:
    """Drop personality_profiles table."""
    op.execute("DROP INDEX IF EXISTS uq_personality_profiles_active")
    op.execute("DROP INDEX IF EXISTS ix_personality_profiles_tenant")
