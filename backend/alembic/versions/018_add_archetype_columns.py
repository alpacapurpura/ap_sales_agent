...
def upgrade() -> None:
    # --- New columns (already idempotent) ---
    ...
    # --- Backfill archetype from existing type (only if type column exists) ---
    op.execute("""
        DO $$
        BEGIN
            IF EXISTS (
    ...
    # --- Backfill format_hint from type (only if type column exists) ---
    op.execute("""
        DO $$
        BEGIN
            IF EXISTS (
    ...
    op.execute("""
        ...
        BEGIN
            IF EXISTS (
                ...
                WHERE offer_value_level = 'level_0_free'
                  AND (is_lead_magnet IS NULL OR is_lead_magnet = FALSE);
            END IF;
        END $$
    """)
