...
from src.core.services.tracing import trace_node
...
def node_state_manager(state: AgentState):
    ...
    try:
        ...
        # Enforce JSON output in the system prompt implicitly or explicitly
        # We append a strict instruction here to ensure reliability
        ...
        decision_raw = LLMFactory.get_service().generate_response(
            ...
            system_prompt=reasoning_prompt + strict_json_instruction,
            model_type="smart",
            temperature=0.2,
...
def node_response_generation(state: AgentState):
    ...
    # --- REFLEXION FEEDBACK INJECTION ---
    critique_feedback = state.get("critique_reasoning")
    is_revision = state.get("is_revision", False)
    
    messages_for_llm = state["messages"].copy()
    
    if is_revision and critique_feedback:
        # Inject the critic's feedback as a high-priority system note at the end
        print(f"🔄 REFLEXION: Generating revision based on feedback: {critique_feedback}")
        messages_for_llm.append({
            "role": "system", 
            "content": f"[CRITICAL INSTRUCTION] Your previous response was rejected. \nFEEDBACK: {critique_feedback}\n\nGenerate a NEW response that addresses this feedback."
        })
    ...
    # --- DYNAMIC PERSONA INJECTION (Specialist Workers) ---
    current_stage = state.get("current_state", "rapport")
    persona_instruction = ""
    
    # Load from Tenant Config or Default
    t_config = state.get("tenant_config", {})
    agent_name = t_config.get("agent_persona", "Visionaria")
    
    if current_stage in ["rapport", "discovery"]:
        persona_instruction = (
            f"You are '{agent_name} Empática'. "
            "TONE: Warm, curious, patient. "
            "GOAL: Build trust. Do NOT sell yet. Validate their feelings. "
            "STYLE: Use open questions. Mirror their language."
        )
    elif current_stage in ["solution", "proposal"]:
        persona_instruction = (
            f"You are '{agent_name} Estratega'. "
            "TONE: Confident, authoritative, visionary. "
            "GOAL: Position the offer as the ONLY solution. "
            "STYLE: Use 'Power Words' (Transformación, Escalar, Libertad)."
        )
    elif current_stage in ["closing", "payment"]:
        persona_instruction = (
            f"You are '{agent_name} Closer'. "
            "TONE: Direct, urgent, professional. "
            "GOAL: Get the commitment. Handle fear with logic + emotion. "
            "STYLE: Short sentences. Clear Call to Action (CTA)."
        )
    
    messages_for_llm.append({
        "role": "system",
        "content": f"[PERSONA OVERRIDE]\n{persona_instruction}\n\nAdopt this persona strictly for the next response."
    })
...
def node_critic(state: AgentState):
    """
    Reflexion Critic Node.
    Evaluates the last generated message for quality, safety, and relevance.
    """
    messages = state.get("messages", [])
    if not messages:
        return {"critique_reasoning": "No messages to critique", "is_revision": False}
        
    last_bot_msg = messages[-1]
    last_bot_content = last_bot_msg.get("content") if isinstance(last_bot_msg, dict) else getattr(last_bot_msg, "content", "")
    
    # Get last user message for context
    last_user_content = ""
    if len(messages) >= 2:
        last_user_msg = messages[-2]
        last_user_content = last_user_msg.get("content") if isinstance(last_user_msg, dict) else getattr(last_user_msg, "content", "")
        
    current_critique_count = state.get("critique_count", 0)
    
    # MAX RETRIES: 2 (To avoid infinite loops and latency)
    if current_critique_count >= 2:
        print(f"⚠️ CRITIC: Max retries ({current_critique_count}) reached. Approving despite potential issues.")
        return {"is_revision": False, "critique_reasoning": None}
        
    try:
        # Render Critic Prompt
        critic_prompt = prompt_loader.render(
            "critic_system.j2",
            current_state=state.get("current_state"),
            user_profile=state.get("user_profile"),
            last_user_message=last_user_content,
            draft_response=last_bot_content
        )
        
        # Call LLM (Fast Model is sufficient for critique)
        critique_json = LLMFactory.get_service().generate_response(
            messages=[],
            system_prompt=critic_prompt,
            model_type="fast",
            temperature=0.1,
            max_output_tokens=500,
            top_p=0.1 # Deterministic
        )
        ...
        try:
            ...
            cleaned_json = critique_json.replace("```json", "").replace("```", "").strip()
            decision = json.loads(cleaned_json)
        except json.JSONDecodeError:
            print(f"CRITIC JSON ERROR: {critique_json}")
            # Fallback: Assume approved if we can't parse
            return {"is_revision": False, "critique_reasoning": None}
            
        status = decision.get("status", "APPROVE")
        score = decision.get("score", 10)
        feedback = decision.get("feedback", "")
        
        if status == "REVISE" and score < 8:
            print(f"❌ CRITIC REJECTED (Score {score}): {feedback}")
            # We need to REMOVE the bad message from history so the generator creates a new one
            # The 'messages' key in the return dict will overwrite or append based on reducer.
            # Since we are using List[Dict] (replace/append manually), we need to be careful.
            # In this architecture, we return state updates. 
            # If we want to 'undo' the last message, we should probably pop it from the list here?
            # Or simpler: Keep it, but mark it as 'draft' in the prompt?
            # BETTER STRATEGY: Remove the last message from the state passed to generator.
            
            # Since we can't easily 'delete' from state in LangGraph without a custom reducer,
            # we will pop it here and return the shortened list.
            new_messages = messages[:-1] 
            
            return {
                "messages": new_messages,
                "critique_reasoning": feedback,
                "critique_count": current_critique_count + 1,
                "is_revision": True
            }
            
        else:
            print(f"✅ CRITIC APPROVED (Score {score})")
            return {
