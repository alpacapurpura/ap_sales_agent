...
from src.core.domain.schema import AIProvider
from src.core.services.llm.base import BaseLLMService
...
class LLMFactory:
    ...
    def get_service_for_tenant(cls, tenant) -> BaseLLMService:
        ...
        raise ValueError("AI Configuration Error: No API Key provided and platform keys are disabled for this tenant.")

    @classmethod
    def _create_service_instance(cls, provider: str, api_key: Optional[str] = None) -> BaseLLMService:
        if provider == AIProvider.OPENAI:
            return OpenAIService(api_key=api_key)
        elif provider == AIProvider.GEMINI:
            from src.core.services.llm.providers.gemini import GeminiService
