...
async def get_user_profile(
    ...
    """
    logger.info("get_user_profile_called", 
...
async def get_team_members(
    ...
    for u in users:
        ...
        results.append(TeamMemberSchema(
            id=str(u.id),
...
async def create_team_member(
    ...
    if existing_user:
         ...
         if db.query(UserTenantModel).filter_by(user_id=existing_user.id, tenant_id=current_user.tenant_id).first():
             ...
             raise HTTPException(status_code=400, detail="El usuario ya existe en el sistema. Contacte al soporte para añadirlo.")
    ...
    try:
        ...
        if "ya existe" in str(e):
             raise HTTPException(status_code=400, detail="El usuario ya existe en el sistema de identidad.")
        else:
