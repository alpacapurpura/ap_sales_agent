        ...
        try:
                ...
                if scope_options.get("product_id"):
                    scope_should.append(
                        models.Filter(
                            must=[
                                models.FieldCondition(
                                    key="scope", match=models.MatchValue(value="OFFER")
                                ),
                                models.FieldCondition(
                                    key="product_id",
                                    match=models.MatchValue(
                                        value=str(scope_options["product_id"])
                                    ),
                                ),
                            ]
                        )
                ...
                if scope_options.get("marketing_asset_id"):
                    scope_should.append(
                        models.Filter(
                            must=[
                                models.FieldCondition(
                                    key="scope", match=models.MatchValue(value="ASSET")
                                ),
                                models.FieldCondition(
                                    key="marketing_asset_id",
                                    match=models.MatchValue(
                                        value=str(scope_options["marketing_asset_id"])
                                    ),
                                ),
                            ]
                        )
            ...
            for item in final_results:
                ...
                context_parts.append(
                    f"- [{category.upper()}] {content} {source_prefix}"
                )
