"""Brand web crawler and HTML/CSS parsing utilities.
...
# ---------------------------------------------------------------------------
# BrandCrawler — stateless HTTP crawler
# ---------------------------------------------------------------------------


class BrandCrawler:
    """Stateless web crawler for brand extraction.
    ...
    async def crawl_content_with_styles(self, url: str) -> str:
        ...
        everything to extract_html_with_styles() for structured extraction.
        """
        try:
            headers = {"User-Agent": self._USER_AGENT}
            async with httpx.AsyncClient(timeout=15.0, follow_redirects=True, headers=headers) as client:
