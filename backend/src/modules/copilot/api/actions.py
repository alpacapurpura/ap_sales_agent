...
async def extract_brand_data(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Internal extraction error: {str(error)}")
...
async def generate_offer_psychology(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"AI Generation failed: {str(error)}")
