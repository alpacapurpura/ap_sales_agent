...
def _format_lead_option(lead_tuple):
    ...
    channel = "Telegram" if getattr(lead, "telegram_id", None) else ("WhatsApp" if getattr(lead, "whatsapp_id", None) else "Web")
...
def render_sales_audit_page():
    ...
    st.markdown("Visualiza las conversaciones de los leads y las trazas de ejecución de los nodos del agente.")
    ...
    try:
        ...
        if hasattr(st, "session_context_tab") and st.session_context_tab:
            with st.sidebar:
                ...
                if st.session_context_tab == "profile":
                    ...
                    last_trace = repo.db.query(AgentTrace).filter(AgentTrace.user_id == lead_id).order_by(AgentTrace.created_at.desc()).first()
                    
                    if last_trace and last_trace.output_state:
                        st.json(last_trace.output_state, expanded=False)
                    else:
                        st.info("No hay trazas o estados registrados aún para este lead.")
        ...
        # Fetch timeline
        timeline = repo.get_full_timeline(lead_id=lead_id, tenant_id=str(tenant_id), limit=200)
        ...
        for event in timeline_asc:
            ...
            if event_type == "message":
                ...
                with st.expander(f"⚙️ Nodo: {node_name} - {exec_time:.0f}ms"):
                    ...
                    tab_in, tab_out, tab_llm = st.tabs(["Input State", "Output State", "LLM Logs"])
                    ...
                    with tab_llm:
                        ...
                        if not has_llm:
                            ...
                            trace_details = repo.get_trace_details(event["id"], str(tenant_id))
                            if trace_details and trace_details.get("llm_logs"):
                                for log in trace_details["llm_logs"]:
                                    st.markdown(f"**Modelo:** `{log['model']}` | **Template:** `{log['prompt_template']}`")
                                    st.markdown(f"**Tokens:** In `{log['tokens']['in']}` / Out `{log['tokens']['out']}`")
                                ...
                                st.warning("No se pudieron cargar los detalles del LLM.")
