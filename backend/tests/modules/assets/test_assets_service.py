...
def _make_storage_mock(storage_path="/tmp/fake/logo.png", public_url="/static/uploads/fake/logo.png"):
    mock = MagicMock()
...
class TestUploadAsset:
    ...
    def test_upload_calls_storage_save(self, db, seed_tenant, tenant_id):
        ...
        svc.upload_asset(tenant_id=tenant_id, file_obj=file_obj, filename="vid.mp4", mime_type="video/mp4")

        assert storage.save.called

    def test_upload_detects_mime_from_filename_when_none_given(self, db, seed_tenant, tenant_id):
        storage = _make_storage_mock()
        svc = _make_service(db, storage)

        file_obj = io.BytesIO(b"data")
        asset = svc.upload_asset(tenant_id=tenant_id, file_obj=file_obj, filename="photo.jpeg")
    ...
    def test_upload_initial_status_is_processing(self, db, seed_tenant, tenant_id):
        ...
        file_obj = io.BytesIO(b"data")
        asset = svc.upload_asset(tenant_id=tenant_id, file_obj=file_obj, filename="img.png", mime_type="image/png")
...
class TestListAssets:
    def test_list_assets_returns_own_assets(self, db, seed_tenant, tenant_id):
        ...
        svc.upload_asset(tenant_id=tenant_id, file_obj=io.BytesIO(b"a"), filename="a.png", mime_type="image/png")
        svc.upload_asset(tenant_id=tenant_id, file_obj=io.BytesIO(b"b"), filename="b.png", mime_type="image/png")

        results = svc.list_assets(tenant_id)
        assert len(results) == 2

    def test_list_assets_tenant_isolation(self, db, seed_tenant, seed_other_tenant, tenant_id, other_tenant_id):
        storage = _make_storage_mock()
        svc = _make_service(db, storage)
    ...
    def test_list_assets_filters_by_type(self, db, seed_tenant, tenant_id):
        ...
        svc.upload_asset(tenant_id=tenant_id, file_obj=io.BytesIO(b"i"), filename="img.png", mime_type="image/png")
        svc.upload_asset(tenant_id=tenant_id, file_obj=io.BytesIO(b"d"), filename="doc.pdf", mime_type="application/pdf")
...
class TestDeleteAsset:
    def test_delete_asset_removes_record(self, db, seed_tenant, tenant_id):
        ...
        asset = svc.upload_asset(tenant_id=tenant_id, file_obj=io.BytesIO(b"x"), filename="x.png", mime_type="image/png")
    ...
    def test_delete_asset_wrong_tenant_returns_false(self, db, seed_tenant, seed_other_tenant, tenant_id, other_tenant_id):
