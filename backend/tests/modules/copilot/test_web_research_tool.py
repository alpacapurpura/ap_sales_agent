...
@pytest.mark.asyncio
async def test_web_research_empty_results():
    with patch(
        "src.modules.copilot.application.tools.interview.web_research.TavilySearchService"
    ) as mock_cls:
        mock_service = MagicMock()
...
@pytest.mark.asyncio
async def test_web_research_default_max_results():
    with patch(
        "src.modules.copilot.application.tools.interview.web_research.TavilySearchService"
    ) as mock_cls:
        mock_service = MagicMock()
...
def test_web_research_is_a_tool():
    """Verify web_research is a LangChain tool with the correct name."""
