...
from tests.modules.offer.conftest import create_product_model, TENANT_A
...
class TestCreateAndRetrieveRoundtrip:
    def test_roundtrip(self, db: Session, tenant_a):
        ...
        model = create_product_model(tenant_a, name="Roundtrip", archetype="programa",
                                     value_level="transformacion", headline_promise="Transform")
...
class TestGetAllExcludesArchived:
    def test_archived_not_in_results(self, db: Session, tenant_a):
        repo = OfferRepository(db)
        db.add_all([create_product_model(tenant_a, name="Active", status="active"),
...
class TestToDomainNormalization:
    ...
    def test_legacy_pricing(self, db: Session, tenant_a):
        repo = OfferRepository(db)
        model = create_product_model(tenant_a, pricing=[{"plan_type": "PAY_IN_FULL", "label": "x", "total_amount": 997}])
        db.add(model); db.flush()
