...
def test_detect_and_accumulate_returns_tuple():
    with patch.object(SemanticRouter, "detect_intent", return_value=("buying_signal", 0.85)):
...
def test_signal_accumulation_buying():
    with patch.object(SemanticRouter, "detect_intent", return_value=("buying_signal", 0.85)):
        intent, score, signals = SemanticRouter.detect_and_accumulate(
            "quiero comprar", existing_signals=[], tenant_id=None
        )
        assert intent == "buying_signal"
...
def test_signal_accumulation_schedule():
    with patch.object(SemanticRouter, "detect_intent", return_value=("schedule_signal", 0.72)):
        intent, score, signals = SemanticRouter.detect_and_accumulate(
...
def test_signal_accumulation_dedup():
    ...
    with patch.object(SemanticRouter, "detect_intent", return_value=("buying_signal", 0.90)):
        intent, score, signals = SemanticRouter.detect_and_accumulate(
...
def test_signal_accumulation_below_threshold():
    with patch.object(SemanticRouter, "detect_intent", return_value=("buying_signal", 0.40)):
        intent, score, signals = SemanticRouter.detect_and_accumulate(
...
def test_signal_accumulation_non_buying_intent():
    with patch.object(SemanticRouter, "detect_intent", return_value=("objection_money", 0.92)):
        intent, score, signals = SemanticRouter.detect_and_accumulate(
...
def test_signal_accumulation_does_not_mutate_original():
    ...
    with patch.object(SemanticRouter, "detect_intent", return_value=("buying_signal", 0.88)):
