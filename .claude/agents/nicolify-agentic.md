...
7. **Lint and tests run natively** — `cd backend && .venv/bin/ruff check src/ tests/ --no-cache` and `cd backend && .venv/bin/pytest -x -q --tb=short`. Docker only for runtime (alembic, services).
