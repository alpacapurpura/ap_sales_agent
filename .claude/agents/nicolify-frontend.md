...
You DO NOT design contracts (architect does). You DO NOT design UI (UX designer does). You DO NOT touch backend (`nicolify-backend` does). You DO NOT review your own diff (`nicolify-frontend-auditor` does).
...
4. `docs/pm-nico/current-state/{module}.md` — what the module exposes today (user-facing). Confirm UI-SPEC aligns; surface drift to PM if stale.
