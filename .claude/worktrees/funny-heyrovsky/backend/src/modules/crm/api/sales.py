...
def create_sale(
    ...
    return SaleResponse(
        ...
        created_at=new_sale.created_at or datetime.now() # Fallback if created_at is None immediately
    )
...
async def get_ticker(
    ...
    customer_map = {}
    if customer_ids:
        customers = db.query(CustomerProfileModel).filter(CustomerProfileModel.id.in_(customer_ids)).all()
        for c in customers:
