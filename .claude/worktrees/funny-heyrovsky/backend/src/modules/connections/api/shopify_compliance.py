...
async def customers_data_request(
    ...
    try:
        ...
        logger.info("shopify_compliance_customers_data_request", 
        ...
        logger.error("shopify_compliance_error", endpoint="customers_data_request", error=str(e))
...
async def customers_redact(
    ...
    try:
        payload = await request.json()
        logger.info("shopify_compliance_customers_redact", 
            shop_domain=payload.get("shop_domain"),
            customer=payload.get("customer")
        )
        ...
        logger.error("shopify_compliance_error", endpoint="customers_redact", error=str(e))
        return {"status": "error", "message": str(e)}
...
async def shop_redact(
    ...
    try:
        payload = await request.json()
        logger.info("shopify_compliance_shop_redact", 
            shop_domain=payload.get("shop_domain")
        )
