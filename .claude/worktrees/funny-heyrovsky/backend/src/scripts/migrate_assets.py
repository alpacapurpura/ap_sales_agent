...
def migrate():
    ...
    with engine.connect() as connection:
        ...
        try:
            ...
            try:
                connection.execute(text("ALTER TABLE offer_gallery_images RENAME TO assets;"))
                print("✅ Table renamed.")
            except Exception as e:
                if "does not exist" in str(e):
                    print("⚠️ Table 'offer_gallery_images' not found. Checking if 'assets' exists...")
            ...
            connection.execute(text("ALTER TABLE assets ALTER COLUMN offer_id DROP NOT NULL;"))
            ...
            for col_name, col_type, default in columns:
                try:
                    connection.execute(text(f"ALTER TABLE assets ADD COLUMN {col_name} {col_type} DEFAULT {default};"))
            ...
            connection.execute(text("UPDATE assets SET storage_path = file_path WHERE storage_path IS NULL;"))
            # Set mime_type default for existing images (assuming they are images)
            connection.execute(text("UPDATE assets SET mime_type = 'image/jpeg' WHERE mime_type IS NULL;")) # Approximation
