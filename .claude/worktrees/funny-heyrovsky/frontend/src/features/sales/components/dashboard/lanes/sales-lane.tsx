            ...
            <CardContent className="flex-1 p-0">
                <ScrollArea className="h-[500px] px-4">
                    <div className="space-y-3 py-4">
                        {sales.map(sale => (
                            <div key={sale.id} className="p-3 bg-white dark:bg-slate-950 rounded-lg border shadow-sm flex justify-between items-center">
                                <div>
                                ...
                                <div className="text-right">
                                    <Badge variant={sale.stage === "EXPANSION" ? "default" : "outline"} className="text-[10px] mb-1 block w-fit ml-auto">
                                        {sale.stage}
                                    </Badge>
                                    <div className="text-[10px] text-muted-foreground truncate max-w-[100px]" title={sale.offer_name || ""}>
                                        {sale.offer_name}
                                    </div>
                                </div>
                            </div>
                        ...
                        {!loading && sales.length === 0 && (
                            <div className="text-center text-sm text-muted-foreground py-10">No sales recorded</div>
                        )}
                        {loading && (
