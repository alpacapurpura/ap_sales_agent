    ...
    <div className="w-full">
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {instructors.map((instructorId, idx) => {
            ...
            if (!instructor) {
                return (
                    <div key={idx} className="flex flex-col items-center text-center p-3 rounded-xl bg-muted/20 border border-transparent border-dashed">
                        ...
                        <div className="w-full px-1 opacity-50">
                            ...
                            <p className="text-[10px] text-muted-foreground truncate font-mono opacity-70">ID: {instructorId.slice(0, 8)}...</p>
                        </div>
                    </div>
                );
            }
            ...
            return (
                <div key={instructor.id} className="flex flex-col items-center text-center p-3 rounded-xl bg-muted/20 border border-transparent hover:border-border/50 transition-colors">
                    <Avatar className="h-14 w-14 mb-2 ring-2 ring-background shadow-sm">
                        <AvatarImage src={instructor.headshot_url} alt={instructor.name} className="object-cover" />
                        <AvatarFallback className="bg-primary/10 text-primary">
                            <User className="h-6 w-6" />
                        </AvatarFallback>
                    </Avatar>
                    <div className="w-full px-1">
                        <p className="text-xs font-semibold truncate text-foreground/90">{instructor.name}</p>
                        <p className="text-[10px] text-muted-foreground truncate font-medium opacity-80">{instructor.role}</p>
                    </div>
                </div>
            );
        })}
