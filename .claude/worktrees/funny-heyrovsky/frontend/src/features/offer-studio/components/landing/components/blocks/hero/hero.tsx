...
function Hero({ layout = "centered", bgImage, className, children, ...props }: HeroProps) {
    ...
    const layoutStyles = {
        ...
        background: "relative min-h-[600px] flex items-center justify-center text-center px-6 overflow-hidden",
    ...
    const containerContent = (
        <HeroContext.Provider value={{ layout, primaryColor }}>
            ...
            <div className={cn(
                layout === "split" ? "max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center" : 
                layout === "vsl" ? "max-w-4xl mx-auto space-y-8" :
                layout === "quiz" ? "max-w-2xl mx-auto space-y-8 bg-white p-8 rounded-2xl shadow-xl border border-slate-100" :
                layout === "background" ? "relative z-20 max-w-4xl mx-auto space-y-8 text-white" :
                layout === "urgency" ? "" : // Urgency wrapper handles its own structure
...
function Content({ className, children }: { className?: string; children: React.ReactNode }) {
    ...
    return (
        <div className={cn(
            layout === "split" ? "space-y-8 text-left order-2 lg:order-1" : 
            layout === "vsl" ? "space-y-4" :
            layout === "background" ? "" :
            "space-y-4",
            className
        )}>
...
// 3. Subheadline
function Subheadline({ className, children, ...props }: React.HTMLAttributes<HTMLParagraphElement>) {
    const { layout } = useHeroContext();
...
function CTA({ className, children, icon = true, ...props }: CTAProps) {
    ...
    const buttonStyles = {
        centered: "text-lg px-10 py-7 rounded-full shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all",
        split: "text-lg px-8 py-6 shadow-lg hover:shadow-xl transition-all",
        background: "text-lg px-10 py-7 rounded-full shadow-xl hover:shadow-2xl hover:scale-105 transition-all bg-white text-slate-900 hover:bg-slate-100 border-none",
    ...
    const style = layout !== "background" && layout !== "quiz" ? { backgroundColor: primaryColor } : {};

    return (
        <Button 
            size="lg" 
            className={cn(buttonStyles[layout], className)}
            style={style}
            {...props}
        >
            {children} {icon && <ArrowRight className={cn("ml-2", layout === "quiz" ? "w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity text-blue-500" : "w-5 h-5")} />}
        </Button>
    );
}
...
function Media({ type, src, className, alt }: MediaProps) {
    ...
    if (layout === "split") {
        return (
            <div className={cn("relative order-1 lg:order-2 h-[400px] lg:h-[600px] rounded-2xl overflow-hidden shadow-2xl rotate-1 hover:rotate-0 transition-transform duration-500", className)}>
                <div 
...
// 6. Badge/Tag
function Tag({ children, className, variant = "outline" }: { children: React.ReactNode, className?: string, variant?: "default" | "secondary" | "destructive" | "outline" }) {
    const { layout } = useHeroContext();
...
function Scarcity({ children, className }: { children: React.ReactNode, className?: string }) {
    ...
    return (
        <div className={cn(
            ...
            layout === "background" ? "mt-6 justify-center text-slate-300 bg-black/30 w-fit mx-auto backdrop-blur-sm" :
            layout === "split" ? "text-amber-600 bg-amber-50 w-fit rounded-md" :
