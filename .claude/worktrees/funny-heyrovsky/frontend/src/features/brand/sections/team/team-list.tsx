        ...
        <div className="grid gap-4">
            {team.map(member => {
                ...
                const imageUrl = member.headshot_url 
                    ? (member.headshot_url.startsWith("http") ? member.headshot_url : `${apiUrl}${member.headshot_url}`)
                    : null;
                        ...
                        <CardContent className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div className="flex items-center space-x-4 overflow-hidden">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-sm text-muted-foreground truncate">{member.role || "Sin rol definido"}</p>
                                    {member.is_primary_voice && <Badge variant="secondary" className="mt-1 text-xs bg-blue-100 text-blue-700 hover:bg-blue-100 border-blue-200">Voz Principal</Badge>}
                                </div>
                            </div>
