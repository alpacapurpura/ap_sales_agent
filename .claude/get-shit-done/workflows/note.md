...
- **Global scope**: `/home/chris/AISALESHT/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/chris/AISALESHT/.claude/notes/`)
...
2. Glob `/home/chris/AISALESHT/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/chris/AISALESHT/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/chris/AISALESHT/.claude/notes/` — works in any directory
