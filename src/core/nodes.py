...
from src.services.router_service import SemanticRouter
import json
import re
...
def node_financial_enforcer(state: AgentState):
    ...
    if last_msg_obj.get("role") != "assistant":
        return state
    ...
    if modified:
        ...
        # Update Redis history for consistency (simple append fix not perfect but ok for now)
        # save_session_history(state["user_id"], "assistant", last_response) # Removed
